...
from aiogram.types import Message, CallbackQuery, ReplyKeyboardMarkup, KeyboardButton, InlineKeyboardMarkup, InputFile
...
async def start_cmd(message: Message):
    ...
    if channels:
        ...
        await send_with_photo(message, "start", welcome_text, main_kb())
...
async def admin_add_required(callback: CallbackQuery, state: FSMContext):
    ...
    kb = back_btn("admin_required_channels")
    edit_colored_keyboard(
        callback.message.chat.id,
        callback.message.message_id,
...
async def admin_add_earn(callback: CallbackQuery, state: FSMContext):
    ...
    kb = back_btn("admin_earn_channels")
    edit_colored_keyboard(
        callback.message.chat.id,
        callback.message.message_id,
        "📝 Введите username канала (без @):",
...
async def admin_add_extra(callback: CallbackQuery, state: FSMContext):
    ...
    kb = back_btn("admin_extra_channels")
    edit_colored_keyboard(
        callback.message.chat.id,
        callback.message.message_id,
        "📝 Введите username канала (без @):",

...
def send_colored_keyboard(chat_id, text, buttons, parse_mode="HTML"):
    """
    Отправляет сообщение с цветными Inline-кнопками через прямой запрос к Telegram API.
    
    buttons: список строк, каждая строка - [{"text": "...", "callback_data": "...", "style": "primary|success|danger|default"}]
    """
    ...
    for row in buttons:
        ...
        for btn in row:
            ...
            # В Telegram цвет кнопки определяется параметром "style"
            btn_data = {
                "text": btn["text"],
...
def edit_colored_keyboard(chat_id, message_id, text, buttons, parse_mode="HTML"):
    """Редактирует сообщение с цветными кнопками"""
    keyboard = {"inline_keyboard": []}
...
async def tasks_menu(message: Message):
    ...
    available = []
    for task in tasks:
        cursor.execute("SELECT id FROM task_executions WHERE task_id = ? AND user_id = ?", (task[0], user_id))
    ...
    if not available:
        ...
        send_colored_keyboard(message.chat.id, "📋 Задания\n\nВы уже выполнили все доступные задания!", kb)
        return
    
    text = "📋 Доступные задания:\n\n"
    for i, task in enumerate(available[:10]):
    ...
    kb = [
        [{"text": "🔄 Обновить", "callback_data": "refresh_tasks", "style": "primary"}],
        [{"text": "➕ Создать задание", "callback_data": "create_task", "style": "success"}],
        [{"text": "🔙 Главное меню", "callback_data": "main_menu", "style": "default"}]
    ]
    send_colored_keyboard(message.chat.id, text, kb)

...
async def earn_cmd(message: Message):
    ...
    if not channels:
        send_colored_keyboard(message.chat.id, "🪙 Пока нет заданий.", [back_btn("main_menu")[0]])
        return
...
async def admin_add_required(callback: CallbackQuery, state: FSMContext):
    ...
    edit_colored_keyboard(
        ...
        back_btn("admin_required_channels")
    )

...
async def start_cmd(message: Message):
    ...
    if channels:
        ...
        send_colored_keyboard(message.chat.id, welcome_text + "\n\n🎁 Получите бонус после обязательных подписок.", kb)
    else:
...
async def check_required(callback: CallbackQuery):
    ...
    if not_sub:
        ...
        kb.append([{"text": "🔙 Главное меню", "callback_data": "main_menu", "style": "default"}])
        edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
        ...
        if not user or user[13] == 0:
            ...
            edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                                  "✅ Вы уже получили бонус.", back_btn("main_menu"))
...
"""
    kb = [[{"text": "🔄 Обновить", "callback_data": "refresh_profile", "style": "primary"}],
...
"""
    ...
    send_colored_keyboard(callback.message.chat.id, text, kb)
    await callback.answer()
...
async def elite_cmd(message: Message):
    ...
    kb = [[{"text": "💎 Купить за 25,000 монет", "callback_data": "buy_elite", "style": "success"}],
...
async def buy_elite(callback: CallbackQuery):
    ...
    backup_db()
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "✅ Elite Sub активирована на 30 дней!", back_btn("main_menu"))
    await callback.answer()
...
📊 Тарифы на услуги:
📱 Telegram: • Подписчики — 21 баллов • Реакции — 25 баллов • Просмотры — 1.5 балла
🎭 VK: В разработке
🎵 TikTok: В разработке
...
"""
    send_colored_keyboard(message.chat.id, text, back_btn("back_to_extra"))
...
"""
    kb = [[{"text": "📤 Пригласить", "callback_data": "invite", "style": "success"}],
...
async def rating_cmd(message: Message):
    kb = [[{"text": "👥 Топ рефералов", "callback_data": "rating_refs", "style": "primary"}],
...
async def transfer_cmd(message: Message, state: FSMContext):
    ...
    send_colored_keyboard(message.chat.id, "💸 Введите ID пользователя:", kb)
...
async def giveaway_cmd(message: Message):
    kb = [[{"text": "🏆 Топ рефералов", "callback_data": "giveaway_top", "style": "primary"}],
...
async def earn_cmd(message: Message):
    ...
    channels = cursor.fetchall()
    if not channels:
        send_colored_keyboard(message.chat.id, "🪙 Пока нет заданий.", back_btn("main_menu"))
        return
    kb = []
    for ch, name in channels:
        kb.append([{"text": f"📢 {name}", "url": f"https://t.me/{ch}", "style": "primary"}])
...
"""
    kb = [[{"text": "📩 Написать", "url": "https://t.me/cf_mz", "style": "primary"}],
          [{"text": "🔙 Назад", "callback_data": "main_menu", "style": "default"}]]
    send_colored_keyboard(message.chat.id, text, kb)
...
"""
    send_colored_keyboard(message.chat.id, text, back_btn("main_menu"))
...
async def tasks_menu(message: Message):
    ...
    if not tasks:
        kb = [[{"text": "➕ Создать задание", "callback_data": "create_task", "style": "success"}],
              [{"text": "🔙 Главное меню", "callback_data": "main_menu", "style": "default"}]]
...
async def create_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    if user[3] < 100:
        ...
        return
    kb = [[{"text": "📱 Подписка", "callback_data": "task_type_subscribe", "style": "primary"}],
    ...
    delete_message(callback.message.chat.id, callback.message.message_id)
    send_colored_keyboard(callback.message.chat.id, "➕ Создание задания\n\nВыберите тип:", kb)
...
✅ {msg}
"""
    kb = [[{"text": "✅ Создать", "callback_data": "task_confirm", "style": "success"}],
...
async def admin_channels(callback: CallbackQuery):
    kb = [
        [{"text": "📢 Обязательные", "callback_data": "admin_required_channels", "style": "primary"}],
        [{"text": "🪙 Спонсоры (Заработать)", "callback_data": "admin_earn_channels", "style": "primary"}],
        [{"text": "👑 Спонсоры (Elite)", "callback_data": "admin_extra_channels", "style": "primary"}],
...
async def admin_required_channels(callback: CallbackQuery):
    ...
    kb = [
        [{"text": "➕ Добавить", "callback_data": "admin_add_required", "style": "success"}],
        [{"text": "➖ Удалить", "callback_data": "admin_remove_required", "style": "danger"}],
        [{"text": "🔙 Назад", "callback_data": "admin_channels", "style": "default"}]
    ]
...
async def admin_add_required(callback: CallbackQuery, state: FSMContext):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "📝 Введите username канала (без @):", back_btn("admin_required_channels"))
...
async def admin_remove_required(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "🗑 Выберите канал для удаления:", kb)
    await callback.answer()
...
async def admin_earn_channels(callback: CallbackQuery):
    ...
    if channels:
        ...
        text += "Пока нет каналов."
    kb = [
        [{"text": "➕ Добавить", "callback_data": "admin_add_earn", "style": "success"}],
        [{"text": "➖ Удалить", "callback_data": "admin_remove_earn", "style": "danger"}],
        [{"text": "🔙 Назад", "callback_data": "admin_channels", "style": "default"}]
    ]
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, kb)
    await callback.answer()
...
async def admin_add_earn(callback: CallbackQuery, state: FSMContext):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "📝 Введите username канала (без @):", back_btn("admin_earn_channels"))
    await state.set_state(AdminState.waiting_channel_username)
    await callback.answer()
...
async def admin_extra_channels(callback: CallbackQuery):
    ...
    if channels:
        ...
        text += "Пока нет каналов."
    kb = [
        [{"text": "➕ Добавить", "callback_data": "admin_add_extra", "style": "success"}],
        [{"text": "➖ Удалить", "callback_data": "admin_remove_extra", "style": "danger"}],
        [{"text": "🔙 Назад", "callback_data": "admin_channels", "style": "default"}]
    ]
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, kb)
    await callback.answer()
...
async def admin_add_extra(callback: CallbackQuery, state: FSMContext):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "📝 Введите username канала (без @):", back_btn("admin_extra_channels"))
    await state.set_state(AdminState.waiting_channel_username)
    await callback.answer()
...
async def admin_promocodes(callback: CallbackQuery):
    kb = [
        [{"text": "➕ Создать промокод", "callback_data": "admin_create_promo", "style": "success"}],
        [{"text": "📋 Список промокодов", "callback_data": "admin_list_promo", "style": "primary"}],
        [{"text": "🔙 Назад", "callback_data": "admin_back", "style": "default"}]
    ]
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "🎟 Управление промокодами:", kb)
    await callback.answer()

@dp.callback_query(lambda c: c.data == "admin_create_promo")
async def admin_create_promo(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "📝 Введите код промокода (латиница, цифры):", back_btn("admin_promocodes"))
...
async def admin_users(callback: CallbackQuery):
    kb = [
        [{"text": "🔍 Найти пользователя", "callback_data": "admin_find_user", "style": "primary"}],
        [{"text": "💰 Выдать монеты", "callback_data": "admin_give_coins", "style": "success"}],
        [{"text": "💸 Забрать монеты", "callback_data": "admin_take_coins", "style": "danger"}],
        [{"text": "💎 Выдать Elite", "callback_data": "admin_give_elite", "style": "success"}],
        [{"text": "💎 Забрать Elite", "callback_data": "admin_take_elite", "style": "danger"}],
        [{"text": "🚫 Заблокировать", "callback_data": "admin_ban_user", "style": "danger"}],
        [{"text": "🔓 Разблокировать", "callback_data": "admin_unban_user", "style": "success"}],
        [{"text": "🔙 Назад", "callback_data": "admin_back", "style": "default"}]
    ]
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "👤 Управление пользователями:", kb)
    await callback.answer()

@dp.callback_query(lambda c: c.data == "admin_give_elite")
async def admin_give_elite(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "💎 Введите ID пользователя для выдачи Elite Sub:", back_btn("admin_users"))
...
async def admin_take_elite(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "💎 Введите ID пользователя для забора Elite Sub:", back_btn("admin_users"))
    await state.set_state(AdminState.waiting_user_id)
...
async def admin_give_coins(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "💰 Введите ID пользователя:", back_btn("admin_users"))
    await state.set_state(AdminState.waiting_user_id)
...
async def admin_ban_user(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "🚫 Введите ID пользователя для блокировки:", back_btn("admin_users"))
    await state.set_state(AdminState.waiting_user_id)
...
async def admin_unban_user(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "🔓 Введите ID пользователя для разблокировки:", back_btn("admin_users"))
    await state.set_state(AdminState.waiting_user_id)
...
async def admin_broadcast(callback: CallbackQuery):
    kb = [
        [{"text": "📢 Всем пользователям", "callback_data": "admin_broadcast_all", "style": "success"}],
        [{"text": "🔙 Назад", "callback_data": "admin_back", "style": "default"}]
    ]
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "📨 Рассылка:\n\nВыберите тип:", kb)
...
async def admin_broadcast_all(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id,
                          "📝 Введите текст рассылки:", back_btn("admin_broadcast"))

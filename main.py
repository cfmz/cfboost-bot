...
async def check_bot_in_channel_async(channel_username):
    try:
        ...
        data = await async_get(url, params={"chat_id": f"@{channel_username}"})
        if not data.get('ok'):
            return False, "❌ Бот не найден в канале. Добавьте бота в канал как администратора."
        url = f"{TELEGRAM_API_URL}/getChatMember"
        data = await async_get(url, params={"chat_id": f"@{channel_username}", "user_id": bot.id})
        if not data.get('ok'):
...
# ========== ФУНКЦИИ ОТПРАВКИ С ЦВЕТНЫМИ КНОПКАМИ ==========
def color_btn(text, callback_data=None, url=None, style="default"):
    """Стили: primary(🔵), success(🟢), danger(🔴), default(⚪)"""
    btn = {"text": text}
    if url:
        btn["url"] = url
    else:
        ...
        if message.text and "start=ref" in message.text:
            try:
                ...
                if referrer and referrer[0] != user_id:
                    ...
                    update_balance(referrer[0], 7500, f"Реферал {user_id} зарегистрировался", "bonus")
                    update_balance(user_id, 1000, "Бонус за регистрацию по реферальной ссылке", "bonus")
...
async def check_required(callback: CallbackQuery):
    ...
    if not_sub:
        ...
        if not user or user[13] == 0:
            ...
            backup_db()
            await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                                  "✅ Вы подписаны на все каналы!\n🎁 Вы получили 5000 баллов!", build_keyboard([back_btn("main_menu")]))
        else:
            await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                                  "✅ Вы уже получили бонус.", build_keyboard([back_btn("main_menu")]))
...
async def elite_cmd(message: Message):
    ...
    if is_elite_active(user[0]):
        text = (
            f"💎 Elite Sub активна до {user[11][:10]}\n\n"
            f"📊 Бонусы:\n"
            f"• -16% на создание\n"
            f"• +16% к награде\n"
            f"• Скидка 2% на пополнение"
        )
    else:
        text = (
            f"💎 Elite Sub — 25,000 монет или 25 звёзд\n\n"
            f"📊 Бонусы:\n"
            f"• -16% на создание\n"
            f"• +16% к награде\n"
            f"• Скидка 2% на пополнение\n"
            f"• Доступ к Elite-заданиям"
        )
...
async def tariffs_cmd(message: Message):
    ...
    text = (
        f"💰 Ваш баланс: {format_number(user[3] if user else 0)} баллов\n\n"
        f"📊 Тарифы на услуги:\n\n"
        f"📱 Telegram:\n"
        f"• Подписчики — 21 баллов\n"
        f"• Реакции — 25 баллов\n"
        f"• Просмотры — 1.5 балла\n\n"
        f"🎭 VK: В разработке\n"
        f"🎵 TikTok: В разработке\n"
        f"📷 Instagram: В разработке\n\n"
        f"👑 Elite подписчики — 300 баллов\n\n"
        f"⚠️ Telegram — пока что единственная доступная платформа."
    )
...
async def referrals_cmd(message: Message):
    ...
    text = (
        ...
        f"━━━━━━━━━━━━━━━\n"
        f"👥 Приглашено: {user[8]}\n"
        f"💰 Заработано: {format_number(user[5])}\n"
        f"🏷 Скидка: {discount}%\n"
        f"До 5% осталось: {remaining} чел.\n"
        f"🔗 Ссылка:\n"
        f"https://t.me/{BOT_USERNAME}?start=ref{user[0]}\n"
        f"📋 Ваши рефералы:\n"
        f"{ref_list if ref_list else 'Пока нет'}"
    )
...
async def invite_cmd(callback: CallbackQuery):
    ...
    text = (
        f"🎁 Привет! Присоединяйся к боту для взаимной накрутки!\n"
        f"💰 Получи 1000 баллов за регистрацию.\n"
        f"🔥 Выполняй задания, зарабатывай монеты!\n"
        f"🔗 Моя ссылка:\n"
        f"https://t.me/{BOT_USERNAME}?start=ref{user[0]}"
    )
...
async def check_earn(callback: CallbackQuery):
    ...
    cursor.execute("SELECT COUNT(*) FROM transactions WHERE user_id = ? AND description LIKE '%спонсор%' COLLATE NOCASE", (user_id,))
    if cursor.fetchone()[0] == 0:
        update_balance(user_id, 3500, "Спонсорские каналы", "earn")
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "✅ +3500 баллов!", build_keyboard([back_btn("main_menu")]))
    else:
...
async def more_cmd(message: Message):
    ...
    text = "👑 ELITE-РАЗДЕЛ\n━━━━━━━━━━━━━━━\n\n"
    text += "⚠️ ПРАВИЛА:\n• Проверяйте задания в течение 2 дней\n• Не злоупотребляйте доработками (максимум 3 раза)\n• После 3-й доработки — авто-зачисление баллов\n• За систематические нарушения — блокировка\n\n"
    ...
    if not sponsor_channels and elite_count == 0:
    ...
    if can_create:
        ...
        kb_rows.append([color_btn("🔒 Купить Elite Sub для создания", "buy_elite", style="primary")])
    ...
    await send_with_photo(message.from_user.id, "more", text, build_keyboard(*kb_rows))
...
async def support_cmd(message: Message):
    text = (
        ...
        f"• Как пополнить?\n"
        f"• Сколько времени?\n"
        f"• Что делать если заказ не выполнился?"
    )
    kb = build_keyboard(
...
async def tasks_menu(message: Message):
    ...
    for i, task in enumerate(tasks[:10]):
        ...
        if free > 0 and not already_done:
            kb_rows.append([color_btn(f"✅ Взять задание #{i+1}", f"take_task_{task_id}", style="success")])
...
async def task_count(message: Message, state: FSMContext):
    ...
    if not is_ok:
        await send_message_api(message.from_user.id, f"❌ {msg}\n\n📌 Инструкция:\n1. Добавьте бота @{BOT_USERNAME} в канал @{channel}\n2. Дайте ему права администратора (минимум: 'Просмотр участников')\n3. После этого попробуйте снова", build_keyboard([back_btn("tasks_menu_back")]))
        await state.clear()
        return
...
async def elite_screenshot_photo(message: Message, state: FSMContext):
    ...
    # Отправляем скриншот заказчику с кнопками
...
async def elite_rework_message(message: Message, state: FSMContext):
    ...
    cursor.execute("UPDATE elite_submissions SET rework_count = ?, rework_message = ?, status = 'rework', updated_at = CURRENT_TIMESTAMP WHERE task_id = ? AND user_id = ?", (rework_count, html.escape(message.text), task_id, user_id))
    conn.commit()
    backup_db()
...
async def elite_task_count(message: Message, state: FSMContext):
    ...
    text = (
        ...
        f"⚠️ ПРАВИЛА:\n"
        f"• Вы можете отправить на доработку максимум 3 раза\n"
        f"• После 3-й доработки баллы зачисляются АВТОМАТИЧЕСКИ\n"
        f"• Злоупотребление доработками — блокировка"
    )
    kb = build_keyboard(
...
# --- Спонсорские каналы (общая функция) ---
...
# --- Обработчик ввода username и названия канала ---
...
async def admin_user_id_input(message: Message, state: FSMContext):
    ...
    if action == "find_user":
        ...
        await send_message_api(message.from_user.id, text, build_keyboard([back_btn("admin_users")]))
        await state.clear()
    ...
    while True:
        try:
            ...
            for exec_id, task_id, user_id, creator_id, link, reward, is_elite in executions:
                # Проверяем, есть ли уже активный отложенный штраф

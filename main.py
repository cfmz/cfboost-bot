...
async def earn_cmd(message: Message):
    ...
    if not channels:
        ...
        await message.answer("🪙 Пока нет заданий.", reply_markup=kb)
        return
...
async def tasks_menu(message: Message):
    ...
    if not tasks:
        ...
        await message.answer("📋 Задания\n\nПока нет активных заданий. Создайте своё!", reply_markup=kb)
        return
...
async def create_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    await callback.message.answer("➕ Создание задания\n\nВыберите тип:", reply_markup=kb)

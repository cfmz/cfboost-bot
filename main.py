...
async def start_cmd(message: Message, state: FSMContext, command: Command = None):
    ...
    if not user:
        ...
        if command and command.args:
            ref = command.args
            if ref.startswith("ref"): ref = ref[3:]
        elif message.text and "start=ref" in message.text:
            try: ref = message.text.split("start=ref")[1].split()[0]
            except: pass
...
async def check_required(callback: CallbackQuery):
    if get_user(callback.from_user.id)[12] == 1:
        await callback.answer("🚫 Вы забанены.", show_alert=True); return
...
async def profile_cmd(message: Message, state: FSMContext):
    await state.clear(); fsm_last_activity.pop(message.from_user.id, None)
...
async def elite_cmd(message: Message, state: FSMContext):
    ...
    kb = build_keyboard([color_btn("💎 Купить за 25,000", "buy_elite", style="success")], [back_btn("main_menu")])
...
async def buy_elite(callback: CallbackQuery):
    ...
    if user[12] == 1: await callback.answer("🚫 Вы забанены.", show_alert=True); return
    if user[3] < 25000: await callback.answer("❌ Недостаточно монет!", show_alert=True); return
...
async def tariffs_cmd(message: Message, state: FSMContext):
    await state.clear(); fsm_last_activity.pop(message.from_user.id, None)
    user = get_user(message.from_user.id)
    text = f"💰 <b>Ваш баланс:</b> {format_number(user[3] if user else 0)} баллов\n\n📊 <b>Тарифы на услуги:</b>\n📱 Telegram:\n• Подписчики — 21 балл\n• Реакции — 25 баллов\n• Просмотры — 1.5 балла\n👑 Elite — 300 баллов"
...
async def promo_process(message: Message, state: FSMContext):
    if get_user(message.from_user.id)[12] == 1:
        await message.answer("🚫 Вы забанены."); await state.clear(); fsm_last_activity.pop(message.from_user.id, None); return
    await update_fsm_activity(message.from_user.id)
    ...
    if not promo:
        await message.answer("❌ Промокод не найден."); await state.clear(); fsm_last_activity.pop(message.from_user.id, None); return
    pid, bonus, max_uses, used = promo
    if max_uses > 0 and used >= max_uses:
        await message.answer("❌ Промокод использован."); await state.clear(); fsm_last_activity.pop(message.from_user.id, None); return
    cursor.execute("SELECT id FROM promocode_activations WHERE promocode_id = ? AND user_id = ?", (pid, user_id))
    if cursor.fetchone():
        await message.answer("❌ Вы уже активировали этот промокод."); await state.clear(); fsm_last_activity.pop(message.from_user.id, None); return
...
async def transaction_history_cmd(message: Message, state: FSMContext):
    ...
    if not txns:
        await message.answer("📊 У вас пока нет транзакций.", reply_markup=main_kb()); return
...
async def cb_main(callback: CallbackQuery, state: FSMContext):
    await state.clear(); fsm_last_activity.pop(callback.from_user.id, None)
    await delete_msg_api(callback.message.chat.id, callback.message.message_id)
...
async def rating_title(callback: CallbackQuery):
    ...
    if next_lvl: text += f"До следующего титула «{next_lvl[1]}» осталось {format_number(max(0, next_lvl[0] - user[4]))} баллов."
    kb = build_keyboard([back_btn("rating_back")])
    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, text, kb)
    await callback.answer()
...
async def transfer_id(message: Message, state: FSMContext):
    ...
    if not get_user(to_id):
        await message.answer("❌ Пользователь с таким ID не найден."); return
...
async def transfer_amount(message: Message, state: FSMContext):
    ...
    if amount < 100:
        await message.answer("❌ Минимальная сумма перевода — 100 баллов."); return
    user = get_user(message.from_user.id)
    commission = math.ceil(amount * 0.02)
    total = amount + commission
    if user[3] < total:
        await message.answer(f"❌ Недостаточно средств. Сумма с комиссией 2%: {total} баллов."); return
...
async def giveaway_top(callback: CallbackQuery):
    if get_user(callback.from_user.id)[12] == 1: await callback.answer("🚫 Вы забанены.", show_alert=True); return
    cursor.execute("SELECT username, referrals_weekly FROM users WHERE is_banned = 0 AND referrals_weekly > 0 ORDER BY referrals_weekly DESC LIMIT 10")
    top = cursor.fetchall()
    medals = ["🥇","🥈","🥉","4.","5.","6.","7.","8.","9.","10."]
...
async def check_earn(callback: CallbackQuery):
    ...
    if not_sub:
        await callback.answer(f"❌ Вы не подписаны на: {', '.join(not_sub)}", show_alert=True); return
...
async def tasks_menu(message: Message, page: int = 0):
    ...
    if not tasks:
        ...
        await send_with_photo(message.from_user.id, "tasks",
...
# ========== ВЗЯТЬ ЗАДАНИЕ (с исправленной транзакцией) ==========
...
async def take_task(callback: CallbackQuery):
    ...
    try:
        ...
        cursor.execute("SELECT id, creator_id, task_type, link, reward_per_unit, max_executors, current_executors, is_elite, status FROM tasks WHERE id = ? AND status = 'active'", (task_id,))
        task = cursor.fetchone()
        if not task:
            cursor.execute("ROLLBACK"); await callback.answer("❌ Задание уже неактивно.", show_alert=True); await tasks_menu(callback.message); return

        task_id, creator_id, task_type, link, reward, max_exec, current_exec, is_elite, status = task

        cursor.execute("SELECT id FROM task_executions WHERE task_id = ? AND user_id = ?", (task_id, user_id))
        if cursor.fetchone():
            cursor.execute("ROLLBACK"); await callback.answer("❌ Вы уже выполнили это задание.", show_alert=True); return
        if current_exec >= max_exec:
            cursor.execute("ROLLBACK"); await callback.answer("❌ Все места уже заняты.", show_alert=True); await tasks_menu(callback.message); return
        ...
        if cursor.rowcount == 0:
            cursor.execute("ROLLBACK"); await callback.answer("❌ Место только что заняли.", show_alert=True); await tasks_menu(callback.message); return
        ...
        if channel:
            try:
                ...
                cursor.execute("ROLLBACK"); await callback.answer(f"❌ Ошибка проверки: {str(e)}", show_alert=True); return

        base_reward = reward
        if is_elite_active(user_id):
            reward = int(reward * 1.16)

        # Обновление баланса без коммита
...
# ========== СОЗДАНИЕ ЗАДАНИЯ (с проверкой бана) ==========
...
async def create_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    if not user: user = create_user(callback.from_user.id)
    if user[3] < 100:
        await callback.answer("❌ Недостаточно монет! Минимум 100 монет.", show_alert=True); return
...
async def task_count(message: Message, state: FSMContext):
    ...
    if not channel:
        ...
        await state.clear(); fsm_last_activity.pop(message.from_user.id, None); return
...
# ========== ВЗЯТЬ ELITE-ЗАДАНИЕ (с исправленной транзакцией и проверкой бана) ==========
...
async def take_elite_task(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        task_id, creator_id, task_type, link, description, reward, max_exec, current_exec, is_elite, status = task
        ...
        # Бонус Elite исполнителю (начисляется при подтверждении, здесь просто занимаем место)
        base_reward = reward
    ...
    await edit_with_photo(
        ...
        f"💰 Награда: {base_reward} монет (базовая)\n"
        f"📝 Описание: {description}",
        kb
    )
    await callback.answer()
...
# ========== МОИ ЗАДАНИЯ / ВЫПОЛНЕНИЯ (с проверкой бана) ==========
...
# ========== СОЗДАНИЕ ELITE-ЗАДАНИЯ (с проверкой бана) ==========
...
async def admin_user_id_input(message: Message, state: FSMContext):
    ...
    if action == "find_user":
        ...
        conn.commit(); backup_db()
        try: await bot.send_message(user_id, "✅ Вы были разблокированы администратором.")
        except: pass
...
async def run_giveaway_and_notify():
    ...
    for i, (uid, refs) in enumerate(winners):
        ...
        try:
            ...
            await bot.send_message(uid, f"🎉 Поздравляем! Вы заняли {medals[i]} место в еженедельном розыгрыше и получаете {prizes[i]} монет!")
        except: pass
    conn.commit()
    backup_db()
...
async def reset_weekly_stats():
    ...
    1. Запускает автоматический розыгрыш (начисление призов топ-3 реферерам).
    2. Обнуляет referrals_weekly и spent_weekly у всех пользователей.

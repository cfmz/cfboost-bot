...
async def more_cmd(message: Message, state: FSMContext):
    ...
    kb_rows = []
    if sponsor_channels:
        kb_rows.append([color_btn("✅ Проверить подписки (спонсоры)", "check_extra_earn", style="success")])
    kb_rows.append([color_btn("📋 Список заданий", "more_tasks_list", style="primary")])
    if can_create:
...
# ========== ВЗЯТЬ ЗАДАНИЕ (с бонусом Elite +16% исполнителю) ==========
...
async def take_task(callback: CallbackQuery):
    ...
    try:
        ...
        base_reward = reward
        # Бонус +16% для Elite-исполнителя
...
# ========== ВЗЯТЬ ELITE-ЗАДАНИЕ (занимает место, создаёт заявку) ==========
...
async def take_elite_task(callback: CallbackQuery, state: FSMContext):
    ...
    cursor.execute("BEGIN IMMEDIATE")
    try:
        # Проверка статуса внутри транзакции
        cursor.execute(
    ...
    await edit_with_photo(
        ...
        f"💰 Награда: {reward} монет (базовая, +16% с Elite Sub)\n"
        f"📝 Описание: {description}",
        kb
    )
    await callback.answer()
...
async def elite_approve(callback: CallbackQuery):
    ...
    # Проверяем, что заявка всё ещё в статусе 'pending' или 'rework'
    cursor.execute(
...
async def admin_exit(callback: CallbackQuery):
    ...
    await callback.message.answer("🔙 Главное меню:", reply_markup=main_kb())
    await callback.answer()
# ========== ФУНКЦИЯ РОЗЫГРЫША (используется в фоне и админке) ==========
...
# ========== ПРОВЕРКА ОТПИСОК (каждые 30 секунд) ==========
async def check_unsubscribes():
    ...
    while True:
        try:
            ...
            # 4. Авто-принятие Elite-заявок (через 2 дня без ответа заказчика, только если есть скриншот)

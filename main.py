...
# ========== ЦВЕТНЫЕ КНОПКИ (поддержка Bot API 9.4) ==========
...
async def edit_with_photo(chat_id, message_id, photo_key, text, kb=None):
    try:
        ...
        logger.error(f"Ошибка редактирования фото {photo_key}: {e}")
        try:
            return await edit_message_api(chat_id, message_id, text, kb)
        except:
            pass
...
# Таймауты FSM (единственное определение)
fsm_last_activity = {}
...
async def start_cmd(message: Message, state: FSMContext, command: Command = None):
    ...
    if not user:
        ...
        if command and command.args:
            ...
            if ref.startswith("ref"):
                ref = ref[3:]
        elif message.text and "start=ref" in message.text:
            try:
    ...
    welcome_text = (
        ...
        f"━━━━━━━━━━━━━━━\n"
        f"💎 <b>Elite Sub</b> — подписка с гарантией 365 дней и бонусами.\n\n"
        f"🏷️ <b>Ваш реферальный код:</b> REF{user_id}\n"
        f"Приглашайте друзей и получайте бонусы!\n\n"
        f"📌 <b>Как начать:</b>\n"
        f"1️⃣ Обязательно подпишитесь на каналы ниже.\n"
...
async def elite_cmd(message: Message):
    ...
    if is_elite_active(user[0]):
        text = (
            f"💎 <b>Elite Sub активна</b>\n"
            f"Действует до: {user[11][:10]}\n\n"
            f"📊 <b>Ваши бонусы:</b>\n"
            f"• Скидка 16% на создание обычных заданий\n"
            f"• Награда за выполнение увеличена на 16%\n"
            f"• Дополнительная скидка 2% на пополнение (в будущем)\n"
            f"• Доступ к Elite-заданиям с повышенной оплатой\n"
        )
    else:
        text = (
            f"💎 <b>Elite Sub — премиум подписка</b>\n"
            f"Стоимость: 25,000 монет\n\n"
            f"📊 <b>Что вы получите:</b>\n"
            f"• Создание заданий со скидкой 16%\n"
            f"• Выполнение заданий с бонусом +16% к награде\n"
            f"• Доступ к Elite-заданиям, где награда от 30₿\n"
            f"• Скидка 2% на пополнение баланса\n\n"
...
async def tariffs_cmd(message: Message):
    ...
    text = (
        ...
        f"📱 Telegram:\n"
        f"• Подписчики — 21 балл за место\n"
        f"• Реакции — 25 баллов за место\n"
        f"• Просмотры — 1.5 балла за место\n\n"
        f"👑 Elite-подписчики — 300 баллов за место\n\n"
        f"⚠️ Telegram — пока единственная доступная платформа."
    )
...
async def referrals_cmd(message: Message):
    ...
    text = (
        ...
        f"━━━━━━━━━━━━━━━\n"
        f"👥 <b>Вы пригласили:</b> {user[8]} чел.\n"
        f"💰 <b>Заработано:</b> {format_number(user[5])} баллов\n"
        f"🏷 <b>Ваша скидка:</b> {discount}%\n"
        f"   (5% за 20 рефералов, 10% за 50, 15% за 100, 25% за 500)\n"
        f"Для первой скидки осталось пригласить: {remaining} чел.\n\n"
        f"🔗 <b>Ваша реферальная ссылка:</b>\n"
        f"https://t.me/{BOT_USERNAME}?start=ref{user[0]}\n\n"
...
async def invite_cmd(callback: CallbackQuery):
    ...
    text = (
        ...
        f"https://t.me/{BOT_USERNAME}?start=ref{user[0]}\n\n"
        f"За каждого приглашённого вы получите 7500 баллов, а друг — 1000 баллов."
    )
...
async def promo_cmd(message: Message, state: FSMContext):
    ...
    await send_with_photo(message.from_user.id, "promo",
                          "🎟 <b>Активация промокода</b>\n\nВведите ваш промокод и получите бонусные монеты.",
                          kb)
...
async def rating_cmd(message: Message):
    """
    Показывает меню рейтинга с тремя категориями:
    - Топ рефералов за неделю
    - Топ трат за неделю
    - Личный титул и прогресс
    """
    kb = build_keyboard(
...
async def rating_refs(callback: CallbackQuery):
    """Топ-10 по количеству приглашённых рефералов за неделю."""
    cursor.execute(
        "SELECT username, referrals_weekly FROM users WHERE is_banned = 0 AND referrals_weekly > 0 ORDER BY referrals_weekly DESC LIMIT 10"
    )
    top = cursor.fetchall()
...
async def rating_spent(callback: CallbackQuery):
    """Топ-10 по потраченным монетам за неделю."""
    cursor.execute(
        "SELECT username, spent_weekly FROM users WHERE is_banned = 0 AND spent_weekly > 0 ORDER BY spent_weekly DESC LIMIT 10"
    )
    top = cursor.fetchall()
    medals = ["🥇", "🥈", "🥉", "4.", "5.", "6.", "7.", "8.", "9.", "10."]
...
async def rating_title(callback: CallbackQuery):
    """Показывает текущий титул пользователя и прогресс до следующего."""
    user = get_user(callback.from_user.id)
    if not user:
        user = create_user(callback.from_user.id)
    level, next_lvl = get_user_level(user[4])
    text = (
        f"👑 <b>Ваш титул:</b> {level[1]}\n"
        f"Потрачено всего: {format_number(user[4])} баллов\n"
    )
    if next_lvl:
...
async def transfer_cmd(message: Message, state: FSMContext):
    """
    Начинает процесс перевода монет другому пользователю.
    Сначала запрашивает ID получателя.
    """
    await update_fsm_activity(message.from_user.id)
    kb = build_keyboard([back_btn("back_to_extra")])
...
async def transfer_id(message: Message, state: FSMContext):
    """Принимает ID получателя и запрашивает сумму."""
    await update_fsm_activity(message.from_user.id)
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        await state.clear()
        fsm_last_activity.pop(message.from_user.id, None)
...
async def transfer_amount(message: Message, state: FSMContext):
    """Принимает сумму, проверяет баланс и выполняет перевод."""
    await update_fsm_activity(message.from_user.id)
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        await state.clear()
        fsm_last_activity.pop(message.from_user.id, None)
    ...
    await message.answer(
        ...
        f"💸 Комиссия: {commission} баллов\n"
        f"👤 Получатель: {to_id}"
    )
    try:
        await bot.send_message(
            to_id,
            f"💰 Вам перевели баллы!\n\n"
            f"👤 Отправитель: @{message.from_user.username or user[0]}\n"
            f"💵 Сумма: +{amount} баллов\n"
            f"📊 Ваш баланс: {format_number(get_user(to_id)[3])} баллов"
        )
    except:
        pass
    await state.clear()
...
async def giveaway_cmd(message: Message):
    """Раздел с информацией о еженедельном розыгрыше."""
    kb = build_keyboard(
    ...
    await send_with_photo(
        ...
        "🥈 2 место — 5,000 монет\n"
        "🥉 3 место — 3,000 монет\n\n"
        "Чем больше приглашённых за неделю, тем выше ваш шанс победить!",
        kb
    )

@dp.callback_query(lambda c: c.data == "giveaway_top")
async def giveaway_top(callback: CallbackQuery):
    """Показывает текущий топ рефералов для розыгрыша."""
    if get_user(callback.from_user.id)[12] == 1:
        await callback.answer("🚫 Вы забанены.", show_alert=True)
        return
    cursor.execute(
...
async def giveaway_rules(callback: CallbackQuery):
    """Показывает правила участия в розыгрыше."""
    text = (
...
async def earn_cmd(message: Message):
    """
    Показывает список спонсорских каналов, за подписку на которые можно получить бонус.
    """
    user_id = message.from_user.id
    ...
    kb_rows = [[color_btn(f"📢 {name}", url=f"https://t.me/{ch}", style="primary")] for ch, name in channels]
    kb_rows.append([color_btn("✅ Проверить подписки", "check_earn", style="success")])
    kb_rows.append([back_btn("main_menu")])
...
async def check_earn(callback: CallbackQuery):
    """Проверяет подписку на спонсорские каналы и начисляет бонус."""
    if get_user(callback.from_user.id)[12] == 1:
        await callback.answer("🚫 Вы забанены.", show_alert=True)
        return
    user_id = callback.from_user.id
...
async def more_cmd(message: Message):
    """Вход в Elite-раздел с описанием правил и возможностей."""
    user_id = message.from_user.id
    user = get_user(user_id)
    if not user:
...
async def support_cmd(message: Message):
    """Показывает контакты поддержки и частые вопросы."""
    text = (
    ...
    kb = build_keyboard(
        [color_btn("📩 Написать в поддержку", url="https://t.me/cf_mz", style="primary")],
        [back_btn("main_menu")]
    )
...
async def menu_cmd(message: Message):
    """Открывает дополнительное меню с расширенными функциями."""
    await send_with_photo(
        message.from_user.id,
        "menu",
        "⚡ <b>Дополнительное меню</b>\n\nЗдесь собраны все дополнительные возможности бота.",
        None
    )
...
async def tasks_menu(message: Message, page: int = 0):
    """
    Показывает список доступных обычных заданий с постраничной навигацией.
    На каждой странице выводится до 5 заданий.
    """
    user_id = message.from_user.id
    user = get_user(user_id)
    if not user:
    ...
    # Получаем общее количество активных заданий
    cursor.execute(
        "SELECT COUNT(*) FROM tasks WHERE status = 'active' AND current_executors < max_executors AND is_elite = 0"
    )
    total = cursor.fetchone()[0]
    ...
    cursor.execute(
        ...
        "FROM tasks WHERE status = 'active' AND current_executors < max_executors AND is_elite = 0 "
        "LIMIT ? OFFSET ?",
        (per_page, offset)
    )
    ...
    if not tasks:
        ...
        await send_with_photo(
            message.from_user.id,
            "tasks",
            "📋 <b>Обычные задания</b>\n\nПока нет активных заданий. Вы можете создать своё!",
            kb
        )
        return
    ...
    for i, task in enumerate(tasks):
        ...
        text += (
            f"{offset+i+1}. {task_type.capitalize()}: {link[:30]}...\n"
            f"   💰 Награда: {reward} монет\n"
            f"   🏷 {elite_label}\n"
            f"   📊 Свободно: {free}/{max_exec}\n\n"
        )
        # Показываем кнопку только если ещё не выполнял и есть места
...
async def refresh_tasks(callback: CallbackQuery):
    """Перезагружает список обычных заданий."""
    await delete_msg_api(callback.message.chat.id, callback.message.message_id)
...
async def tasks_page(callback: CallbackQuery):
    """Переход по страницам обычных заданий."""
...
async def tasks_menu_back(callback: CallbackQuery, state: FSMContext = None):
    """Возврат в список заданий из подменю."""
    if state:
        await state.clear()
...
# ========== ВЗЯТЬ ЗАДАНИЕ (с блокировкой БД и бонусом Elite) ==========
@dp.callback_query(lambda c: c.data.startswith("take_task_"))
async def take_task(callback: CallbackQuery):
    """
    Безопасное выполнение задания:
    - Проверка всех условий внутри транзакции
    - Атомарное увеличение счётчика с проверкой лимита
    - Бонус +16% к награде для обладателей Elite Sub
    """
    user_id = callback.from_user.id
    ...
    try:
        ...
        # Для Elite-заданий требуется подписка
        ...
        # Атомарно занимаем место (с проверкой, что ещё не переполнено)
        cursor.execute(
        ...
        # Сохраняем базовую награду для уведомления создателя
        base_reward = reward

        # Применяем бонус Elite исполнителю (+16% к награде)
        if is_elite_active(user_id):
            reward = int(reward * 1.16)

        update_balance(user_id, reward, f"Выполнение задания #{task_id}", "earn")
        cursor.execute(
            "INSERT INTO task_executions (task_id, user_id, is_checked, is_verified) VALUES (?, ?, 0, 1)",
            (task_id, user_id)
        )
    ...
    # Уведомление создателя задания (с базовой наградой)
...
async def create_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    await send_with_photo(
        ...
        "create_task",
        "➕ <b>Создание задания</b>\n\nВыберите тип задания, которое хотите создать.",
        kb
    )
...
async def task_type_selected(callback: CallbackQuery, state: FSMContext):
    """Сохраняет выбранный тип задания и запрашивает ссылку."""
    await update_fsm_activity(callback.from_user.id)
...
async def task_link(message: Message, state: FSMContext):
    """Принимает ссылку и запрашивает описание."""
    await update_fsm_activity(message.from_user.id)
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        await state.clear()
        fsm_last_activity.pop(message.from_user.id, None)
...
async def task_description(message: Message, state: FSMContext):
    """Принимает описание и запрашивает количество исполнителей."""
    await update_fsm_activity(message.from_user.id)
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        await state.clear()
        fsm_last_activity.pop(message.from_user.id, None)
...
async def task_count(message: Message, state: FSMContext):
    """
    Финальная валидация перед подтверждением:
    - Проверка корректности количества
    - Извлечение канала из ссылки
    - Запрет задания на самого бота
    - Проверка дубликата активного задания у пользователя
    - Проверка, что бот добавлен в канал
    - Расчёт стоимости с учётом скидки Elite Sub
    - Показ итогового подтверждения
    """
    await update_fsm_activity(message.from_user.id)
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        await state.clear()
    ...
    # Проверка на дубликат активного задания
    cursor.execute(
        "SELECT id FROM tasks WHERE creator_id = ? AND link = ? AND status = 'active'",
        (message.from_user.id, link)
    )
    if cursor.fetchone():
    ...
    # Применяем скидку Elite Sub для создателя (-16%)
    ...
    if user[3] < total_cost:
        await message.answer(
            f"❌ Недостаточно монет! Нужно {format_number(total_cost)} монет.\n"
            f"💰 Ваш баланс: {format_number(user[3])} монет"
        )
        await state.clear()
        fsm_last_activity.pop(message.from_user.id, None)
        return
    ...
    if is_elite_active(message.from_user.id):
        text += "💎 Скидка Elite Sub 16% уже учтена.\n"
...
async def task_confirm(callback: CallbackQuery, state: FSMContext):
    """Подтверждение создания задания и списание монет."""
    await update_fsm_activity(callback.from_user.id)
    user_id = callback.from_user.id
    data = await state.get_data()
    task_type = data['task_type']
    ...
    # Скидка Elite Sub для создателя
    if is_elite_active(user_id):
        total_cost = int(total_cost * 0.84)
    ...
    # Сохраняем фактическую стоимость для точного возврата при удалении
    ...
    cursor.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (user_id, task_type, link, description, reward_per, count, 0, actual_cost)
    )
    conn.commit()
    backup_db()
...
async def more_tasks_list(callback: CallbackQuery):
    """
    Показывает список Elite-заданий с постраничной навигацией.
    Elite-задания доступны только обладателям активной Elite Sub.
    """
    user_id = callback.from_user.id
    if get_user(user_id)[12] == 1:
        await callback.answer("🚫 Вы забанены.", show_alert=True)
...
# ========== ВЗЯТЬ ELITE-ЗАДАНИЕ (с блокировкой и бонусом) ==========
...
async def take_elite_task(callback: CallbackQuery, state: FSMContext):
    """
    Взятие Elite-задания:
    - Требуется активный Elite Sub
    - Блокировка БД для предотвращения переполнения
    - Бонус +16% к награде при наличии Elite Sub у исполнителя
    """
    user_id = callback.from_user.id
    if get_user(user_id)[12] == 1:
        await callback.answer("🚫 Вы забанены.", show_alert=True)
    ...
    await edit_with_photo(
        ...
        f"💰 Награда: {reward} монет (базовая: {base_reward})\n"
        f"📝 Описание: {description}",
        kb
    )
    await callback.answer()
...
async def elite_submit(callback: CallbackQuery, state: FSMContext):
    """Активирует состояние ожидания скриншота."""
...
async def elite_screenshot_photo(message: Message, state: FSMContext):
    """Принимает фото-скриншот, обновляет заявку и уведомляет заказчика."""
    await update_fsm_activity(message.from_user.id)
    data = await state.get_data()
...
async def elite_screenshot_doc(message: Message, state: FSMContext):
    """Принимает документ как скриншот."""
    await update_fsm_activity(message.from_user.id)
    data = await state.get_data()
    task_id = data['task_id']
    user_id = message.from_user.id
...
async def elite_screenshot_other(message: Message, state: FSMContext):
    """Обработка неверного ввода в режиме ожидания скриншота."""
    await update_fsm_activity(message.from_user.id)
...
async def elite_approve(callback: CallbackQuery):
    """Заказчик подтверждает выполнение Elite-задания."""
    ...
    cursor.execute(
        "UPDATE elite_submissions SET status = 'approved', updated_at = CURRENT_TIMESTAMP WHERE task_id = ? AND user_id = ?",
        (task_id, user_id)
    )
    conn.commit()
    backup_db()
...
async def elite_rework(callback: CallbackQuery, state: FSMContext):
    """Заказчик отправляет задание на доработку (максимум 3 раза)."""
    parts = callback.data.split("_")
    task_id = int(parts[2])
    user_id = int(parts[3])
    ...
    if rework_count >= 3:
        # Авто-зачисление при превышении лимита доработок
        commission = math.ceil(reward * 0.1)
        final_reward = reward - commission
        update_balance(user_id, final_reward, f"Elite-задание #{task_id} авто-принято (превышен лимит доработок)", "earn")
        cursor.execute(
            "UPDATE elite_submissions SET status = 'auto_approved', updated_at = CURRENT_TIMESTAMP WHERE task_id = ? AND user_id = ?",
            (task_id, user_id)
        )
        conn.commit()
        backup_db()
...
async def elite_rework_message(message: Message, state: FSMContext):
    """Отправляет сообщение с доработкой исполнителю."""
    await update_fsm_activity(message.from_user.id)
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        await state.clear()
        fsm_last_activity.pop(message.from_user.id, None)
...
async def elite_message(callback: CallbackQuery, state: FSMContext):
    """Заказчик может отправить произвольное сообщение исполнителю."""
    parts = callback.data.split("_")
    task_id = int(parts[2])
    user_id = int(parts[3])
...
async def elite_send_message(message: Message, state: FSMContext):
    """Отправляет введённое сообщение исполнителю."""
    await update_fsm_activity(message.from_user.id)
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        await state.clear()
        fsm_last_activity.pop(message.from_user.id, None)
...
async def more_my_tasks(callback: CallbackQuery):
    """Показывает созданные пользователем Elite-задания."""
    user_id = callback.from_user.id
    cursor.execute(
...
async def more_my_executions(callback: CallbackQuery):
    """Показывает выполнения Elite-заданий текущим пользователем."""
    user_id = callback.from_user.id
    cursor.execute(
...
async def more_back(callback: CallbackQuery):
    """Возврат в основное меню Elite-раздела."""
...
async def create_elite_task_start(callback: CallbackQuery, state: FSMContext):
    """Начало создания Elite-задания (только для обладателей Elite Sub)."""
    user_id = callback.from_user.id
    if not is_elite_active(user_id):
        await callback.answer("❌ Требуется активная Elite Sub!", show_alert=True)
        return
...
async def elite_type_selected(callback: CallbackQuery, state: FSMContext):
    """Сохраняет тип и запрашивает ссылку."""
    await update_fsm_activity(callback.from_user.id)
...
async def elite_task_count(message: Message, state: FSMContext):
    """Проверка количества и вывод подтверждения."""
    await update_fsm_activity(message.from_user.id)
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        await state.clear()
        fsm_last_activity.pop(message.from_user.id, None)
    ...
    # Скидка Elite Sub для создателя (-16%)
    if is_elite_active(message.from_user.id):
        total_cost = int(total_cost * 0.84)
...
async def elite_task_confirm(callback: CallbackQuery, state: FSMContext):
    """Подтверждение и создание Elite-задания."""
    await update_fsm_activity(callback.from_user.id)
    user_id = callback.from_user.id
    data = await state.get_data()
    task_type = data['task_type']
    ...
    cursor.execute(
        "INSERT INTO tasks (creator_id, task_type, link, description, reward_per_unit, max_executors, is_elite, actual_cost) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (user_id, task_type, link, description, reward, count, 1, actual_cost)
    )
    conn.commit()
    backup_db()
...
async def admin_cmd(message: Message):
    ...
    kb = build_keyboard(
        [
            color_btn("📢 Каналы", "admin_channels", style="primary"),
            color_btn("🎟 Промокоды", "admin_promocodes", style="primary")
        ],
        [
            color_btn("📋 Задания", "admin_tasks", style="primary"),
            color_btn("👤 Пользователи", "admin_users", style="primary")
        ],
        [
            color_btn("📨 Рассылка", "admin_broadcast", style="primary"),
            color_btn("💰 Настройки", "admin_settings", style="primary")
        ],
        [
            color_btn("🏆 Розыгрыш", "admin_giveaway", style="primary"),
            color_btn("📊 Статистика", "admin_stats", style="primary")
        ],
...
async def admin_channels_menu(callback: CallbackQuery):
    """Меню управления всеми типами каналов."""
    kb = build_keyboard(
...
async def admin_required_channels_list(callback: CallbackQuery):
    """Просмотр списка обязательных каналов."""
...
async def admin_add_required_start(callback: CallbackQuery, state: FSMContext):
    """Добавление обязательного канала (запрос username)."""
...
async def admin_remove_required_list(callback: CallbackQuery):
    ...
    kb_rows = [
        [color_btn(f"🗑 {name} (@{username})", f"admin_remove_required_{ch_id}", style="danger")]
        for ch_id, username, name in channels
    ]
...
async def show_sponsor_channels(callback: CallbackQuery, ch_type: str):
    """Универсальная функция для отображения спонсорских каналов."""
...
async def admin_sponsor_remove_list(callback: CallbackQuery):
    ...
    kb_rows = [
        [color_btn(f"🗑 {name} (@{username})", f"{prefix}_remove_{ch_id}", style="danger")]
        for ch_id, username, name in channels
    ]
...
async def admin_promo_bonus_input(message: Message, state: FSMContext):
    ...
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        ...
        await message.answer("🔙 Главное меню:", reply_markup=main_kb())
        return
    try: bonus = int(message.text.strip())
    except:
        await message.answer("❌ Введите число.")
        return
...
async def admin_delete_promo_list(callback: CallbackQuery):
    ...
    if not promos:
        ...
        return
    kb_rows = [
        [color_btn(f"🗑 {code} ({bonus}₿)", f"admin_delete_promo_{pid}", style="danger")]
...
async def admin_users_menu(callback: CallbackQuery):
    kb = build_keyboard(
        [color_btn("🔍 Найти", "admin_find_user", style="primary")],
        [color_btn("💰 Выдать монеты", "admin_give_coins", style="success"),
         color_btn("💸 Забрать монеты", "admin_take_coins", style="danger")],
        [color_btn("💎 Выдать Elite", "admin_give_elite", style="success"),
         color_btn("💎 Забрать Elite", "admin_take_elite", style="danger")],
        [color_btn("🚫 Бан", "admin_ban_user", style="danger"),
         color_btn("🔓 Разбан", "admin_unban_user", style="success")],
        [back_btn("admin_back")]
    )
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
...
async def admin_broadcast_menu(callback: CallbackQuery):
    ...
    try:
        ...
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "📨 <b>Рассылка</b>", kb)
    await callback.answer()
...
async def admin_tasks_show(callback: CallbackQuery):
    ...
    if tasks:
        ...
        for task in tasks:
            ...
            kb_rows.append([color_btn(f"⚙️ Управлять #{tid}", f"admin_task_detail_{tid}", style="primary")])
...
async def check_unsubscribes():
    """
    Бесконечный цикл, который каждые 30 секунд проверяет:
    1. Все выполненные задания (is_verified=1) на предмет отписки исполнителя от канала.
    2. Отложенные штрафы — если прошло 10 минут с момента отписки, применяет штраф или прощает.
    3. Авто-закрытие просроченных заданий (5 дней для обычных, 365 для Elite).
    4. Авто-принятие Elite-заявок, которые висят без ответа заказчика более 2 дней.
    5. Каждые 100 циклов вызывает сборщик мусора для предотвращения утечек памяти.
    """
    cycle_count = 0
    while True:
        try:
            # 1. Проверяем выполнения заданий на отписки
            ...
            for exec_id, task_id, user_id, creator_id, link, reward, is_elite in executions:
                # Пропускаем, если по этому выполнению уже есть активный отложенный штраф
                ...
                if not is_subscribed:
                    # Создаём отложенный штраф на 10 минут
            ...
            # 2. Проверяем отложенные штрафы (не прошло ли 10 минут?)
            ...
            for pen_id, user_id, task_id, exec_id, creator_id, reward, channel, start_time in pending:
                ...
                if (datetime.now() - start).total_seconds() >= 600:  # 10 минут истекли
            ...
            # 3. Закрываем просроченные задания
            ...
            # 4. Авто-принятие Elite-заданий (через 2 дня без ответа заказчика)
            ...
            # Очистка памяти каждые 100 циклов (50 минут)
            if cycle_count >= 100:
                gc.collect()
                cycle_count = 0
...
# ---- СБРОС НЕДЕЛЬНОЙ СТАТИСТИКИ + АВТО-РОЗЫГРЫШ (каждый понедельник в 00:00) ----
async def reset_weekly_stats():
    """
    Каждую минуту проверяет время. В понедельник 00:00:
    1. Обнуляет referrals_weekly и spent_weekly у всех пользователей.
    2. Запускает автоматический розыгрыш (начисление призов топ-3 реферерам).
    3. Засыпает на сутки, чтобы не срабатывать повторно в течение дня.
    """
    while True:
        now = datetime.now()
        if now.weekday() == 0 and now.hour == 0 and now.minute == 0:
            ...
            # Запускаем автоматический розыгрыш
            await run_giveaway_and_notify()
...
# ---- ТАЙМАУТЫ FSM (авто-сброс состояний при бездействии) ----
async def check_fsm_timeouts():
    """
    Каждые 30 секунд проверяет словарь fsm_last_activity.
    Если пользователь не проявлял активность в FSM-состоянии более 10 минут,
    его состояние сбрасывается и ему отправляется уведомление.
    """
    while True:
        now = datetime.now()
        to_clear = [uid for uid, t in list(fsm_last_activity.items()) if (now - t).total_seconds() > 600]
...
async def handle_any_message(message: Message, state: FSMContext):
    """
    Самый последний обработчик в цепочке. Срабатывает на любое сообщение,
    которое не попало под другие фильтры.
    - Если пользователь находится в FSM-состоянии — не вмешиваемся.
    - Если пользователь не зарегистрирован — создаём запись и предлагаем /start.
    - Если забанен — сообщаем.
    - В остальных случаях рекомендуем использовать меню.
    """
    if await state.get_state():
        return
...
async def main():
    """
    Главная точка входа:
    - Удаляет вебхук для перехода на поллинг.
    - Запускает три фоновые задачи: проверка отписок, сброс статистики с розыгрышем, таймауты FSM.
    - Запускает бесконечный поллинг обновлений от Telegram.
    """

...
def backup_db():
    ...
    if transaction_counter >= BACKUP_INTERVAL:
        try:
            if os.path.exists(DB_FILE):
                shutil.copy2(DB_FILE, BACKUP_FILE)
                transaction_counter = 0
                logger.info("✅ Бэкап БД выполнен")
...
def extract_channel_from_link(link):
    # Поддерживает t.me/username, t.me/+hash, t.me/joinchat/...
...
async def transfer_cmd(message: Message, state: FSMContext):
    ...
    await send_with_photo(message, "transfer", "💸 Введите ID пользователя:", kb)
...
async def more_cmd(message: Message):
    ...
    if not is_elite_active(user_id):
        ...
        await send_with_photo(message, "more", "🔒 Доступно только с Elite Sub.", kb)
        return
    
    # Показываем спонсорские каналы
    ...
    kb = [
        ...
        [InlineKeyboardButton(text="➕ Создать Elite-задание", callback_data="create_elite_task")],
        [InlineKeyboardButton(text="📤 Мои задания", callback_data="more_my_tasks")],
        [InlineKeyboardButton(text="📥 Мои выполнения", callback_data="more_my_executions")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu")]
    ]
...
async def create_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    await async_delete_message(callback.message.chat.id, callback.message.message_id)
    await send_with_photo(callback, "create_task", "➕ Создание задания\n\nВыберите тип:", kb)
...
# ---- СПИСОК ELITE-ЗАДАНИЙ (В РАЗДЕЛЕ БОЛЬШЕ ЗАДАНИЙ) ----
...
async def take_elite_task(callback: CallbackQuery, state: FSMContext):
    ...
    if cursor.fetchone():
        ...
        return
    # Создаём запись о взятии задания
...
async def elite_rework(callback: CallbackQuery, state: FSMContext):
    ...
    if rework_count >= 3:
        # Авто-зачисление после 4-й доработки
        cursor.execute("SELECT reward_per_unit FROM tasks WHERE id = ?", (task_id,))
        task = cursor.fetchone()
        if task:
            reward = task[0]
            ...
            await async_delete_message(callback.message.chat.id, callback.message.message_id)
            return
    # Запрашиваем сообщение о доработке
    await state.update_data(task_id=task_id, user_id=user_id, rework_count=rework_count)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="more_tasks_list")]
    ])
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите сообщение для исполнителя (что нужно исправить):", kb)
    await state.set_state(EliteSubmitState.waiting_screenshot)  # Переиспользуем состояние
    await callback.answer()

# ---- ОБРАБОТКА СООБЩЕНИЯ ДЛЯ ДОРАБОТКИ ----
@dp.message(EliteSubmitState.waiting_screenshot, F.text)
...
async def elite_message(callback: CallbackQuery, state: FSMContext):
    ...
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите сообщение для исполнителя:", kb)
    await state.set_state(EliteSubmitState.waiting_screenshot)
    await callback.answer()
...
# ========== ПРОВЕРКА ОТПИСОК (СОХРАНЕНИЕ В БД) ==========
async def check_unsubscribes():
    while True:
        try:
            # Получаем активные выполнения
            ...
            for exec_id, task_id, user_id, creator_id, link, reward, is_elite in executions:
                # Проверяем, есть ли уже активный штраф
            ...
            # Проверяем истекшие штрафы (10 минут)
            ...
            for pen_id, user_id, task_id, exec_id, creator_id, reward, channel, start_time in pending:
                ...
                if (datetime.now() - start).total_seconds() >= 600:
                    # Проверяем, подписался ли обратно
                    try:
                        member = await bot.get_chat_member(f"@{channel}", user_id)
                        is_subscribed = member.status not in ['left', 'kicked']
                    except:
            ...
            # Авто-принятие Elite-заданий через 2 дня

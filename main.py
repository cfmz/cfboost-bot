...
def backup_db():
    try:
        if os.path.exists(DB_FILE): shutil.copy2(DB_FILE, BACKUP_FILE)
    except: pass
...
def is_elite_active(user_id):
    ...
    try: return datetime.strptime(user[11], "%Y-%m-%d %H:%M:%S") > datetime.now()
    except: return False

def get_user_level(total_spent):
    levels = [(0, "Новичок", 0), (50000, "Инвестор", 5000), (200000, "Магнат", 20000),
              (500000, "Легенда", 50000), (1000000, "Властелин Раскрутки", 100000)]
    current = levels[0]
    next_lvl = None
    for i, (threshold, title, reward) in enumerate(levels):
        if total_spent >= threshold:
            current = (threshold, title, reward)
            if i < len(levels)-1: next_lvl = levels[i+1]
...
def extract_channel_from_link(link):
    ...
    if link.startswith('@'): return link[1:]
    return None

def is_admin(user_id): return user_id == ADMIN_ID

def get_referral_discount(ref_count):
    if ref_count >= 500: return 25
    elif ref_count >= 100: return 15
    elif ref_count >= 50: return 10
    elif ref_count >= 20: return 5
    return 0
...
async def check_bot_in_channel_async(channel_username):
    try:
        ...
        if not data.get('ok'): return False, "❌ Бот не найден в канале."
        data = await async_get(f"{TELEGRAM_API_URL}/getChatMember", {"chat_id": f"@{channel_username}", "user_id": bot.id})
        if not data.get('ok'): return False, "❌ Бот не администратор."
        member = data.get('result', {})
        if member.get('status') not in ['administrator','creator']: return False, "❌ Нет прав администратора."
...
def color_btn(text, callback_data=None, url=None, style="default"):
    btn = {"text": text}
    if url: btn["url"] = url
    else: btn["callback_data"] = callback_data
    return btn
...
async def delete_msg_api(chat_id, message_id):
    try: await async_post(f"{TELEGRAM_API_URL}/deleteMessage", json_data={"chat_id": chat_id, "message_id": message_id})
    except: pass
...
async def send_with_photo(chat_id, photo_key, text, kb=None):
    ...
    if not photo_url:
        if kb: return await send_message_api(chat_id, text, kb)
        return await send_message_api(chat_id, text)
    try:
        if kb: return await send_photo_api(chat_id, photo_url, text, kb)
        else: return await send_photo_api(chat_id, photo_url, text)
    except Exception as e:
        logger.error(f"Ошибка фото {photo_key}: {e}")
        if kb: return await send_message_api(chat_id, text, kb)
        return await send_message_api(chat_id, text)
...
async def edit_with_photo(chat_id, message_id, photo_key, text, kb=None):
    try:
        ...
        logger.error(f"Ошибка edit {photo_key}: {e}")
        try: return await edit_message_api(chat_id, message_id, text, kb)
        except: pass

# Состояния
class PromoState(StatesGroup): waiting_code = State()
class TransferState(StatesGroup): waiting_id = State(); waiting_amount = State()
class TaskState(StatesGroup): waiting_type = State(); waiting_link = State(); waiting_description = State(); waiting_count = State(); waiting_confirmation = State()
class EliteTaskState(StatesGroup): waiting_type = State(); waiting_link = State(); waiting_description = State(); waiting_reward = State(); waiting_count = State(); waiting_confirmation = State()
class EliteSubmitState(StatesGroup): waiting_screenshot = State(); waiting_rework_message = State(); waiting_custom_message = State()
class AdminState(StatesGroup): waiting_channel_username = State(); waiting_channel_name = State(); waiting_promo_code = State(); waiting_promo_bonus = State(); waiting_promo_uses = State(); waiting_user_id = State(); waiting_user_amount = State(); waiting_broadcast_text = State()
...
def admin_only(func):
    ...
    return wrapper
# ========== /start с deep_link (реферальная ссылка) ==========
@dp.message(Command("start", magic=F.args))
async def start_with_ref(message: Message, state: FSMContext, command: Command = None):
    await state.clear()
    ...
    if not user:
        ...
        # Парсинг из текста на случай обычного /start (без deep_link)
        if message.text and "start=ref" in message.text:
            try:
                ref = message.text.split("start=ref")[1].split()[0]
                cursor.execute("SELECT user_id FROM users WHERE ref_code = ?", (ref,))
                referrer = cursor.fetchone()
                if referrer and referrer[0] != user_id and not user[7]:  # ещё не приглашён
                    cursor.execute("UPDATE users SET referred_by = ? WHERE user_id = ?", (referrer[0], user_id))
                    update_balance(referrer[0], 7500, f"Реферал {user_id}", "bonus")
    ...
    welcome_text = (
        ...
        f"📷 Instagram — лайки, просмотры\n▶️ YouTube — лайки, просмотры\n\n"
        f"━━━━━━━━━━━━━━━\n💎 Elite Sub\nПодписчики с гарантией 365 дней\n\n"
...
# ========== СБРОС FSM ПРИ /start (если не deep_link) ==========
@dp.message(Command("start"))
async def start_plain(message: Message, state: FSMContext):
    await state.clear()
    fsm_last_activity.pop(message.from_user.id, None)
    await start_with_ref(message, state, None)  # без параметров
...
async def check_required(callback: CallbackQuery):
    ...
    for ch, name in channels:
        try:
            ...
            if member.status in ['left', 'kicked']: not_sub.append(name)
        except: not_sub.append(name)
    ...
    if not_sub:
        ...
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                              f"❌ Не подписаны: {', '.join(not_sub)}", build_keyboard(*kb_rows))
    else:
        user = get_user(user_id)
        if not user or user[13] == 0:
            ...
            backup_db()
            await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                                  "✅ +5000 баллов!", build_keyboard([back_btn("main_menu")]))
        else:
            await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                                  "✅ Бонус уже получен.", build_keyboard([back_btn("main_menu")]))
...
async def show_profile(user_id, chat_id, edit_msg_id=None):
    ...
    text = (
        ...
        f"#️⃣ ID: {user[0]}\n👑 Титул: {level[1]}\n💎 Elite: {elite}\n"
        f"━━━━━━━━━━━━━━━\n"
        f"💰 Баланс: {format_number(user[3])} баллов\n"
        f"💸 Потрачено: {format_number(user[4])} баллов\n"
        f"👥 Рефералов: {user[8]}\n🏷 Скидка: {discount}%\n"
    ...
    kb = build_keyboard(
        [color_btn("🔄 Обновить", "refresh_profile", style="primary")],
        [back_btn("main_menu")]
    )
    if edit_msg_id:
...
async def elite_cmd(message: Message):
    ...
    if is_elite_active(user[0]):
        ...
        text = "💎 Elite Sub — 25,000 монет\n\n📊 Бонусы:\n• -16% на создание\n• +16% к награде\n• Скидка 2%\n• Доступ к Elite-заданиям"
    
    kb = build_keyboard(
        [color_btn("💎 Купить за 25,000", "buy_elite", style="success")],
        [back_btn("main_menu")]
    )
...
async def buy_elite(callback: CallbackQuery):
    ...
    if not user: user = create_user(callback.from_user.id)
    if user[3] < 25000:
    ...
    backup_db()
    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                          "✅ Elite Sub на 30 дней!", build_keyboard([back_btn("main_menu")]))
    await callback.answer()
...
async def tariffs_cmd(message: Message):
    user = get_user(message.from_user.id)
    text = f"💰 Баланс: {format_number(user[3] if user else 0)} баллов\n\n📊 Тарифы:\n📱 Telegram:\n• Подписчики — 21 балл\n• Реакции — 25 баллов\n• Просмотры — 1.5 балла\n👑 Elite — 300 баллов"
...
async def promo_cmd(message: Message, state: FSMContext):
    ...
    kb = build_keyboard([back_btn("back_to_extra")])
    await send_with_photo(message.from_user.id, "promo", "🎟 Введите промокод:", kb)
...
async def promo_process(message: Message, state: FSMContext):
    ...
    conn.commit()
    backup_db()
    await message.answer(f"✅ +{bonus} баллов!")
    await state.clear()
    fsm_last_activity.pop(message.from_user.id, None)

# ========== ИСТОРИЯ ТРАНЗАКЦИЙ ==========
@dp.message(F.text == "💳 История")
async def transaction_history_cmd(message: Message):
    user_id = message.from_user.id
    ...
    if not txns:
        await message.answer("📊 Нет транзакций.", reply_markup=main_kb())
        return
...
async def rating_cmd(message: Message):
    ...
    await send_with_photo(message.from_user.id, "rating", "🏆 Выберите категорию:", kb)
...
async def transfer_cmd(message: Message, state: FSMContext):
    await update_fsm_activity(message.from_user.id)
    kb = build_keyboard([back_btn("back_to_extra")])
    await send_message_api(message.from_user.id, "💸 Введите ID пользователя:", kb)
...
async def transfer_id(message: Message, state: FSMContext):
    ...
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        ...
        await message.answer("🔙 Главное меню:", reply_markup=main_kb())
        return
    try: to_id = int(message.text.strip())
    except:
...
async def transfer_amount(message: Message, state: FSMContext):
    ...
    if user[3] < total:
        await message.answer(f"❌ Недостаточно. Нужно {total} (комиссия 2%).")
        return
...
async def giveaway_cmd(message: Message):
    kb = build_keyboard(
        [color_btn("🏆 Топ рефералов", "giveaway_top", style="primary")],
        [color_btn("📋 Условия", "giveaway_rules", style="primary")],
        [back_btn("back_to_extra")]
    )
    await send_with_photo(message.from_user.id, "giveaway", "🎉 Розыгрыш:", kb)
...
async def earn_cmd(message: Message):
    ...
    if not channels:
        kb = build_keyboard([back_btn("main_menu")])
        await send_with_photo(user_id, "earn", "🪙 Пока нет заданий.", kb)
        return
    
    kb_rows = [[color_btn(f"📢 {name}", url=f"https://t.me/{ch}", style="primary")] for ch, name in channels]
    kb_rows.append([color_btn("✅ Проверить", "check_earn", style="success")])
    kb_rows.append([back_btn("main_menu")])
    
    text = "🪙 Выполните задания:\nНаграда: 3500 монет\n\n"
    for ch, name in channels: text += f"• {name} (@{ch})\n"
...
async def check_earn(callback: CallbackQuery):
    ...
    if cursor.fetchone()[0] == 0:
        ...
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "✅ +3500!", build_keyboard([back_btn("main_menu")]))
    else:
...
async def more_cmd(message: Message):
    user_id = message.from_user.id
    user = get_user(user_id)
    if not user: user = create_user(user_id)
    ...
    text = "👑 ELITE-РАЗДЕЛ\n━━━━━━━━━━━━━━━\n\n⚠️ ПРАВИЛА:\n• Проверяйте задания в течение 2 дней\n• Доработки — макс. 3 раза\n• После 3-й — авто-зачисление\n• Злоупотребление — блокировка\n\n"
    if sponsor_channels:
        text += "📢 СПОНСОРСКИЕ КАНАЛЫ:\n"
        for ch, name in sponsor_channels: text += f"• {name} (@{ch})\n"
        text += "\n"
    text += f"📋 ELITE-ЗАДАНИЙ: {elite_count} шт.\n"
    
    can_create = is_elite_active(user_id)
    kb_rows = [[color_btn("📋 Список заданий", "more_tasks_list", style="primary")]]
    if can_create:
        kb_rows.append([color_btn("➕ Создать Elite-задание", "create_elite_task", style="success")])
    else:
        kb_rows.append([color_btn("🔒 Купить Elite Sub", "buy_elite", style="primary")])
    kb_rows.append([color_btn("📤 Мои задания", "more_my_tasks", style="primary")])
    kb_rows.append([color_btn("📥 Мои выполнения", "more_my_executions", style="primary")])
    kb_rows.append([back_btn("main_menu")])
...
async def support_cmd(message: Message):
    ...
    kb = build_keyboard(
        [color_btn("📩 Написать", url="https://t.me/cf_mz", style="primary")],
        [back_btn("main_menu")]
    )
...
async def menu_cmd(message: Message):
    await send_with_photo(message.from_user.id, "menu", "⚡ Дополнительное меню:", None)
...
async def tasks_menu(message: Message, page: int = 0):
    ...
    if not tasks:
        kb = build_keyboard(
            [color_btn("➕ Создать задание", "create_task", style="success")],
            [back_btn("main_menu")]
        )
        await send_with_photo(message.from_user.id, "tasks", "📋 Задания\n\nПока нет активных заданий. Создайте своё!", kb)
        return
    
    text = f"📋 Доступные задания (стр. {page+1}/{total_pages}):\n\n"
    kb_rows = []
    for i, task in enumerate(tasks):
        ...
        if free > 0 and not already_done:
            kb_rows.append([color_btn(f"✅ Взять задание #{task_id}", f"take_task_{task_id}", style="success")])
    
    # Кнопки пагинации
    nav_buttons = []
    if page > 0:
        nav_buttons.append(color_btn("⬅️ Назад", f"tasks_page_{page-1}", style="primary"))
    if page < total_pages - 1:
        nav_buttons.append(color_btn("➡️ Вперёд", f"tasks_page_{page+1}", style="primary"))
    if nav_buttons:
        kb_rows.append(nav_buttons)
    
    kb_rows.append([color_btn("🔄 Обновить", "refresh_tasks", style="primary")])
    kb_rows.append([color_btn("➕ Создать задание", "create_task", style="success")])
    kb_rows.append([back_btn("main_menu")])
    await send_with_photo(message.from_user.id, "tasks", text, build_keyboard(*kb_rows))
...
# ========== ВЗЯТЬ ЗАДАНИЕ (с блокировкой БД) ==========
...
async def take_task(callback: CallbackQuery):
    ...
    # БЛОКИРОВКА: транзакция с атомарной проверкой
    cursor.execute("BEGIN IMMEDIATE")
    try:
        cursor.execute("SELECT id, creator_id, task_type, link, reward_per_unit, max_executors, current_executors, is_elite FROM tasks WHERE id = ? AND status = 'active'", (task_id,))
        ...
        if is_elite and not is_elite_active(user_id):
            cursor.execute("ROLLBACK")
            kb = build_keyboard([color_btn("💎 Купить Elite Sub", "buy_elite", style="success")])
            await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                                  "🔒 Это Elite-задание. Требуется подписка Elite Sub.", kb)
            await callback.answer()
            return
        ...
        if channel:
            try:
                ...
                if member.status in ['left', 'kicked']:
                    ...
                    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                                          f"❌ Вы не подписаны на канал @{channel}. Подпишитесь и попробуйте снова.", kb)
                    await callback.answer()
                    return
            except Exception as e:
                cursor.execute("ROLLBACK")
    ...
    if creator:
        try:
            await bot.send_message(creator_id, f"✅ НОВОЕ ВЫПОЛНЕНИЕ ЗАДАНИЯ!\n\n📋 Задание #{task_id}\n👤 Исполнитель: @{callback.from_user.username or user_id}\n💰 Награда: {reward} монет\n📊 Прогресс: {current_exec + 1}/{max_exec}\n🔗 Ссылка: {link}")
        except: pass
    
    kb = build_keyboard([back_btn("tasks_menu_back")])
    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                          f"✅ ВЫПОЛНЕНО! +{reward} монет!\n\n📋 Задание #{task_id}\n📊 Осталось мест: {max_exec - (current_exec + 1)}", kb)
    
    try:
        await bot.send_message(user_id, f"✅ Вы выполнили задание #{task_id}!\n💰 Награда: +{reward} монет\n📊 Осталось мест: {max_exec - (current_exec + 1)}")
    except: pass
    await callback.answer()
...
async def create_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    kb = build_keyboard(
        [color_btn("📱 Подписка", "task_type_subscribe", style="primary")],
        [color_btn("❤️ Лайк", "task_type_like", style="primary")],
        [color_btn("👁 Просмотр", "task_type_view", style="primary")],
        [back_btn("tasks_menu_back")]
    )
    await delete_msg_api(callback.message.chat.id, callback.message.message_id)
    await send_with_photo(callback.message.chat.id, "create_task", "➕ Создание задания\n\nВыберите тип:", kb)
...
async def task_type_selected(callback: CallbackQuery, state: FSMContext):
    ...
    kb = build_keyboard([back_btn("tasks_menu_back")])
    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "📎 Введите ссылку (канал, пост или видео):", kb)
...
async def task_count(message: Message, state: FSMContext):
    ...
    text = (
        ...
        f"Количество: {count}\n"
        f"Стоимость за шт: {price_per} монет\n"
        f"Награда за шт: {reward_per} монет\n"
        f"Итого: {format_number(total_cost)} монет\n"
        f"✅ {msg}"
    )
    kb = build_keyboard(
        [color_btn("✅ Создать", "task_confirm", style="success")],
        [color_btn("❌ Отмена", "task_cancel", style="danger")]
    )
...
async def task_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    if user[3] < total_cost:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "❌ Недостаточно монет! Баланс изменился.", build_keyboard([back_btn("main_menu")]))
        await state.clear()
        fsm_last_activity.pop(callback.from_user.id, None)
        await callback.answer()
        return
    ...
    await edit_message_api(callback.message.chat.id, callback.message.message_id, "✅ Задание создано! Ожидайте исполнителей.", build_keyboard([back_btn("main_menu")]))
    await state.clear()
    fsm_last_activity.pop(callback.from_user.id, None)
    await callback.answer()
...
async def task_cancel(callback: CallbackQuery, state: FSMContext):
    await edit_message_api(callback.message.chat.id, callback.message.message_id, "❌ Создание задания отменено.", build_keyboard([back_btn("tasks_menu_back")]))
    await state.clear()
    fsm_last_activity.pop(callback.from_user.id, None)
    await callback.answer()
...
async def more_tasks_list(callback: CallbackQuery):
    ...
    cursor.execute("SELECT id, creator_id, task_type, link, description, reward_per_unit, max_executors, current_executors, is_elite FROM tasks WHERE status = 'active' AND current_executors < max_executors AND is_elite = 1 LIMIT ? OFFSET ?", (per_page, offset))
    tasks = cursor.fetchall()
    ...
    for i, task in enumerate(tasks):
        task_id, creator_id, task_type, link, description, reward, max_exec, current_exec, is_elite = task
        free = max_exec - current_exec
        cursor.execute("SELECT id FROM elite_submissions WHERE task_id = ? AND user_id = ? AND status IN ('pending', 'rework')", (task_id, user_id))
        already_taken = cursor.fetchone()
        text += f"{offset+i+1}. {task_type.capitalize()}: {link[:30]}...\n   💰 Награда: {reward} монет\n   📊 Свободно: {free}/{max_exec}\n"
        if not already_taken and free > 0:
            kb_rows.append([color_btn(f"✅ Взять задание #{task_id}", f"take_elite_task_{task_id}", style="success")])

    # Кнопки пагинации
    nav_buttons = []
    if page > 0:
        nav_buttons.append(color_btn("⬅️ Назад", f"more_tasks_page_{page-1}", style="primary"))
    if page < total_pages - 1:
        nav_buttons.append(color_btn("➡️ Вперёд", f"more_tasks_page_{page+1}", style="primary"))
    if nav_buttons:
        kb_rows.append(nav_buttons)

    kb_rows.append([color_btn("🔄 Обновить", "more_tasks_list", style="primary")])
...
# ========== ВЗЯТЬ ELITE-ЗАДАНИЕ (с блокировкой) ==========
...
async def take_elite_task(callback: CallbackQuery, state: FSMContext):
    ...
    cursor.execute("BEGIN IMMEDIATE")
    try:
        cursor.execute("SELECT id, creator_id, task_type, link, description, reward_per_unit, max_executors, current_executors, is_elite FROM tasks WHERE id = ? AND status = 'active'", (task_id,))
        task = cursor.fetchone()
        if not task:
            cursor.execute("ROLLBACK")
            await callback.answer("❌ Задание уже неактивно.", show_alert=True)
        ...
        cursor.execute("INSERT INTO elite_submissions (task_id, user_id, status) VALUES (?, ?, 'pending')", (task_id, user_id))
        cursor.execute("COMMIT")
        conn.commit()
        backup_db()
    ...
    kb = build_keyboard(
        [color_btn("📸 Отправить скриншот", f"elite_submit_{task_id}", style="success")],
        [back_btn("more_tasks_list")]
    )
    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                          f"👑 Задание #{task_id} взято!\n\n📌 Требуется отправить скриншот выполнения.\n💰 Награда: {reward} монет\n📝 Описание: {description}", kb)
    await callback.answer()
...
async def elite_submit(callback: CallbackQuery, state: FSMContext):
    ...
    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                          "📸 Отправьте скриншот выполнения задания (фото или документ):", kb)
...
async def elite_screenshot_photo(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_photo(creator_id, file_id, caption=f"📸 Новый скриншот от @{message.from_user.username or user_id}\n\n📋 Задание #{task_id}\n💰 Награда: {reward} монет\n\nПодтвердите выполнение или отправьте на доработку.", reply_markup=kb_for_creator)
    except:
        try:
            await bot.send_message(creator_id, f"📸 Новый скриншот от @{message.from_user.username or user_id}\n\n📋 Задание #{task_id}\n💰 Награда: {reward} монет\n\nПодтвердите выполнение или отправьте на доработку.", reply_markup=kb_for_creator)
        except: pass
...
async def elite_screenshot_doc(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_document(creator_id, file_id, caption=f"📸 Новый скриншот от @{message.from_user.username or user_id}\n\n📋 Задание #{task_id}\n💰 Награда: {reward} монет\n\nПодтвердите выполнение или отправьте на доработку.", reply_markup=kb_for_creator)
    except:
        try:
            await bot.send_message(creator_id, f"📸 Новый скриншот от @{message.from_user.username or user_id}\n\n📋 Задание #{task_id}\n💰 Награда: {reward} монет\n\nПодтвердите выполнение или отправьте на доработку.", reply_markup=kb_for_creator)
        except: pass
...
async def elite_screenshot_other(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() in ["отмена", "🔙 отмена", "назад", "🔙 назад"]:
        ...
        return
    await message.answer("❌ Отправьте скриншот (фото или документ).\nИли напишите 'Отмена' для возврата.")

# ========== ПОДТВЕРЖДЕНИЕ ELITE-ЗАДАНИЯ ==========
...
async def elite_approve(callback: CallbackQuery):
    ...
    try:
        await bot.send_message(user_id, f"✅ Заказчик подтвердил выполнение задания #{task_id}!\n💰 Награда: +{final_reward} монет (комиссия 10%)")
    except: pass
    await delete_msg_api(callback.message.chat.id, callback.message.message_id)

# ========== ДОРАБОТКА ELITE-ЗАДАНИЯ ==========
...
async def elite_rework(callback: CallbackQuery, state: FSMContext):
    ...
    if rework_count >= 3:
        ...
        cursor.execute("UPDATE elite_submissions SET status = 'auto_approved', updated_at = CURRENT_TIMESTAMP WHERE task_id = ? AND user_id = ?", (task_id, user_id))
        conn.commit()
        backup_db()
        await callback.answer("✅ Превышен лимит доработок. Баллы автоматически зачислены исполнителю.", show_alert=True)
        try:
            await bot.send_message(user_id, f"✅ Баллы зачислены! Заказчик превысил лимит доработок.\n💰 Награда: +{final_reward} монет (комиссия 10%)")
        except: pass
        await delete_msg_api(callback.message.chat.id, callback.message.message_id)
        return
    ...
    kb = build_keyboard([back_btn("more_tasks_list")])
    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                          "📝 Введите сообщение для исполнителя (что нужно исправить):", kb)
...
async def elite_rework_message(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(user_id, f"🔄 Заказчик отправил на доработку ({rework_count}/3):\n\n{message.text}{warning}")
    except: pass
...
async def elite_message(callback: CallbackQuery, state: FSMContext):
    ...
    kb = build_keyboard([back_btn("more_tasks_list")])
    await edit_with_photo(callback.message.chat.id, callback.message.message_id, None,
                          "📝 Введите сообщение для исполнителя:", kb)
...
async def elite_send_message(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(user_id, f"📝 Сообщение от заказчика по заданию #{task_id}:\n\n{message.text}")
    except: pass
...
async def more_my_tasks(callback: CallbackQuery):
    user_id = callback.from_user.id
    cursor.execute("SELECT id, task_type, link, description, reward_per_unit, max_executors, current_executors, status FROM tasks WHERE creator_id = ? AND is_elite = 1 ORDER BY id DESC LIMIT 10", (user_id,))
    tasks = cursor.fetchall()
    if not tasks:
        await callback.answer("❌ У вас нет созданных Elite-заданий.", show_alert=True)
        return
    text = "📤 МОИ ELITE-ЗАДАНИЯ (ЗАКАЗЧИК):\n\n"
    for task in tasks:
...
# ========== МОИ ВЫПОЛНЕНИЯ (ИСПОЛНИТЕЛЬ) ==========
@dp.callback_query(lambda c: c.data == "more_my_executions")
async def more_my_executions(callback: CallbackQuery):
    user_id = callback.from_user.id
    cursor.execute("SELECT es.id, es.task_id, t.task_type, t.link, es.status, es.rework_count FROM elite_submissions es JOIN tasks t ON es.task_id = t.id WHERE es.user_id = ? ORDER BY es.id DESC LIMIT 10", (user_id,))
    submissions = cursor.fetchall()
    if not submissions:
        await callback.answer("❌ У вас нет выполненных Elite-заданий.", show_alert=True)
        return
    text = "📥 МОИ ВЫПОЛНЕНИЯ (ИСПОЛНИТЕЛЬ):\n\n"
...
async def create_elite_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    kb = build_keyboard(
        [color_btn("📱 Подписка", "elite_type_subscribe", style="primary")],
        [color_btn("❤️ Лайк", "elite_type_like", style="primary")],
        [color_btn("👁 Просмотр", "elite_type_view", style="primary")],
        [back_btn("more_back")]
    )
    await delete_msg_api(callback.message.chat.id, callback.message.message_id)
    await send_with_photo(callback.message.chat.id, "create_task", "👑 Создание Elite-задания\n\nВыберите тип:", kb)
...
async def elite_task_description(message: Message, state: FSMContext):
    ...
    if message.text in ["🔙 Главное меню", "🔙 Назад"]:
        ...
        return
    await state.update_data(description=safe_html(message.text))
    await message.answer("💰 Введите награду для исполнителя (мин. 30₿):")
...
async def elite_task_count(message: Message, state: FSMContext):
    ...
    text = (
        f"👑 ПОДТВЕРЖДЕНИЕ ELITE-ЗАДАНИЯ\n━━━━━━━━━━━━━━━\n\n"
        f"Тип: {task_type.capitalize()}\nСсылка: {link}\nОписание: {description}\n"
        f"Количество: {count}\nНаграда: {reward}₿\nЦена заказчика: 250₿ × {count} = {total_cost}₿\n"
        f"Комиссия: 10% от награды\n\n"
        f"⚠️ ПРАВИЛА:\n• Максимум 3 доработки\n• После 3-й — авто-зачисление\n• Злоупотребление — блокировка"
    )
    kb = build_keyboard(
        [color_btn("✅ Создать", "elite_task_confirm", style="success")],
        [color_btn("❌ Отмена", "elite_task_cancel", style="danger")]
    )
    await send_message_api(message.from_user.id, text, kb)
...
async def elite_task_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    user = get_user(user_id)
    if user[3] < total_cost:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "❌ Недостаточно монет!", build_keyboard([back_btn("main_menu")]))
        await state.clear()
        fsm_last_activity.pop(callback.from_user.id, None)
        await callback.answer()
        return
    ...
    cursor.execute("INSERT INTO tasks (creator_id, task_type, link, description, reward_per_unit, max_executors, is_elite) VALUES (?, ?, ?, ?, ?, ?, ?)",
                   (user_id, task_type, link, description, reward, count, 1))
    conn.commit()
    backup_db()

    await edit_message_api(callback.message.chat.id, callback.message.message_id, "✅ Elite-задание создано! Ожидайте исполнителей.", build_keyboard([back_btn("main_menu")]))
    await state.clear()
    fsm_last_activity.pop(callback.from_user.id, None)
    await callback.answer()
...
async def elite_task_cancel(callback: CallbackQuery, state: FSMContext):
    await edit_message_api(callback.message.chat.id, callback.message.message_id, "❌ Создание отменено.", build_keyboard([back_btn("more_back")]))
    await state.clear()
    fsm_last_activity.pop(callback.from_user.id, None)
    await callback.answer()
# ========== АДМИН-ПАНЕЛЬ ==========
...
async def admin_cmd(message: Message):
    ...
    kb = build_keyboard(
        [color_btn("📢 Каналы", "admin_channels", style="primary"), color_btn("🎟 Промокоды", "admin_promocodes", style="primary")],
        [color_btn("📋 Задания", "admin_tasks", style="primary"), color_btn("👤 Пользователи", "admin_users", style="primary")],
        [color_btn("📨 Рассылка", "admin_broadcast", style="primary"), color_btn("💰 Настройки", "admin_settings", style="primary")],
        [color_btn("🏆 Розыгрыш", "admin_giveaway", style="primary"), color_btn("📊 Статистика", "admin_stats", style="primary")],
        [color_btn("🔙 Выход", "admin_exit", style="danger")]
    )
    await send_with_photo(message.from_user.id, "admin", "⚙️ Админ-панель:", kb)
...
async def admin_channels_menu(callback: CallbackQuery):
    ...
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, "admin", "📢 Управление каналами:\n\nВыберите тип:", kb)
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "📢 Управление каналами:\n\nВыберите тип:", kb)
    await callback.answer()
...
async def admin_add_required_start(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "📝 Введите username канала (без @):", kb)
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "📝 Введите username канала (без @):", kb)
...
async def admin_remove_required_list(callback: CallbackQuery):
    ...
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "🗑 Выберите канал для удаления:", build_keyboard(*kb_rows))
    except:
...
# --- Обработчики ввода username и названия канала (общие) ---
...
async def admin_promocodes_menu(callback: CallbackQuery):
    ...
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "🎟 Управление промокодами:", kb)
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "🎟 Управление промокодами:", kb)
    await callback.answer()
...
async def admin_create_promo_start(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "📝 Введите код промокода (латиница, цифры):", kb)
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "📝 Введите код промокода (латиница, цифры):", kb)
...
async def admin_delete_promo_list(callback: CallbackQuery):
    ...
    for pid, code, bonus, max_uses, used, is_active in promos:
        ...
        kb_rows.append([color_btn(f"{status} {code} ({bonus}₿) — удалить", f"admin_delete_promo_{pid}", style="danger")])
    kb_rows.append([back_btn("admin_promocodes")])

    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "🗑 Выберите промокод для удаления:", build_keyboard(*kb_rows))
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "🗑 Выберите промокод для удаления:", build_keyboard(*kb_rows))
    await callback.answer()
...
async def admin_users_menu(callback: CallbackQuery):
    kb = build_keyboard(
        [color_btn("🔍 Найти пользователя", "admin_find_user", style="primary")],
        [color_btn("💰 Выдать монеты", "admin_give_coins", style="success"), color_btn("💸 Забрать монеты", "admin_take_coins", style="danger")],
        [color_btn("💎 Выдать Elite", "admin_give_elite", style="success"), color_btn("💎 Забрать Elite", "admin_take_elite", style="danger")],
        [color_btn("🚫 Заблокировать", "admin_ban_user", style="danger"), color_btn("🔓 Разблокировать", "admin_unban_user", style="success")],
        [back_btn("admin_back")]
    )
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "👤 Управление пользователями:", kb)
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "👤 Управление пользователями:", kb)
    await callback.answer()
...
async def admin_user_action_start(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "🔍 Введите ID пользователя:", kb)
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "🔍 Введите ID пользователя:", kb)
...
async def admin_user_id_input(message: Message, state: FSMContext):
    ...
    if action == "find_user":
        text = (
            ...
            f"💸 Потрачено: {format_number(user[4])} баллов\n"
            f"👥 Рефералов: {user[8]}\n"
            f"💎 Elite: {'✅ Активна' if is_elite_active(user[0]) else '❌ Не активна'}\n"
            f"🚫 Забанен: {'✅ Да' if user[12] else '❌ Нет'}"
        )
        kb = build_keyboard([back_btn("admin_users")])
        await send_message_api(message.from_user.id, text, kb)
        await state.clear()
...
async def admin_broadcast_menu(callback: CallbackQuery):
    kb = build_keyboard(
        [color_btn("📢 Всем пользователям", "admin_broadcast_all", style="primary")],
        [back_btn("admin_back")]
    )
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "📨 Рассылка:\n\nВыберите тип:", kb)
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "📨 Рассылка:\n\nВыберите тип:", kb)
    await callback.answer()
...
async def admin_broadcast_all_start(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, "📝 Введите текст рассылки:", kb)
    except:
        await edit_message_api(callback.message.chat.id, callback.message.message_id, "📝 Введите текст рассылки:", kb)
...
async def admin_stats_show(callback: CallbackQuery):
    ...
    text = (
        ...
        f"💰 Всего заработано: {format_number(earned)} баллов\n"
        f"💸 Всего потрачено: {format_number(spent)} баллов"
    )
...
async def admin_settings_show(callback: CallbackQuery):
    text = (
        f"💰 Настройки бота:\n"
        f"📱 Telegram цены:\n"
        f"• Подписка: 21₿ (покупка) / 15₿ (награда)\n"
        f"• Лайк: 5₿ / 3₿\n"
        f"• Просмотр: 3₿ / 1.5₿\n"
        f"💎 Elite Sub: 25,000₿ или 25⭐\n"
        f"🎁 Спонсор награда: 3,500₿\n"
        f"💸 Комиссия перевода: 2%"
    )
...
async def admin_giveaway_menu(callback: CallbackQuery):
    text = (
        ...
        f"• Розыгрыш проводится каждую неделю\n"
        f"• Победители: 1 место - 10,000 монет, 2 место - 5,000, 3 место - 3,000\n"
        f"• Топ обновляется в реальном времени\n"
        f"• Сброс каждый понедельник в 00:00"
    )

    kb = build_keyboard(
        [color_btn("🏆 Запустить вручную", "admin_run_giveaway", style="success")],
        [back_btn("admin_back")]
    )
    try:
        await edit_with_photo(callback.message.chat.id, callback.message.message_id, None, text, kb)
...
async def admin_run_giveaway(callback: CallbackQuery):
    ...
    for i, (user_id, refs) in enumerate(winners):
        user = get_user(user_id)
        name = f"@{user[1]}" if user and user[1] else f"ID: {user_id}"
        text += f"{medals[i]} {name} — {refs} рефералов\n"
...
@admin_only
async def admin_tasks_show(callback: CallbackQuery):
    ...
    if tasks:
        ...
        kb_rows = []
        for task in tasks:
            task_id, creator_id, task_type, link, status, current, max_exec, is_elite = task
            elite_mark = "👑" if is_elite else "📋"
            text += f"{elite_mark} #{task_id} {task_type}: {link[:25]}...\n  Статус: {status} | {current}/{max_exec} | ID:{creator_id}\n\n"
            kb_rows.append([color_btn(f"⚙️ Управлять #{task_id}", f"admin_task_detail_{task_id}", style="primary")])
...
async def admin_task_detail(callback: CallbackQuery):
    ...
    cursor.execute("SELECT id, creator_id, task_type, link, description, reward_per_unit, max_executors, current_executors, is_elite, status, created_at FROM tasks WHERE id = ?", (task_id,))
    task = cursor.fetchone()
    if not task:
        await callback.answer("❌ Задание не найдено.", show_alert=True)
        return
    ...
    elite_mark = "👑 Elite" if is_elite else "📋 Обычное"

    text = (
        f"⚙️ Управление заданием #{tid}\n"
        f"━━━━━━━━━━━━━━━\n"
        f"Тип: {ttype} | {elite_mark}\n"
        f"Ссылка: {link}\n"
        f"Описание: {desc}\n"
        f"Награда: {reward}₿\n"
        ...
        f"Дата: {created[:10] if created else '—'}\n"
        f"━━━━━━━━━━━━━━━"
    )

    kb_rows = []
    if status == 'active':
        kb_rows.append([color_btn("✅ Завершить", f"admin_task_complete_{tid}", style="success")])
        kb_rows.append([color_btn("⏸ Приостановить", f"admin_task_pause_{tid}", style="danger")])
    elif status == 'paused':
        kb_rows.append([color_btn("▶️ Возобновить", f"admin_task_resume_{tid}", style="success")])
    kb_rows.append([color_btn("🗑 Удалить", f"admin_task_delete_{tid}", style="danger")])
...
async def admin_task_complete(callback: CallbackQuery):
    ...
    conn.commit()
    backup_db()
    await callback.answer("✅ Задание завершено!", show_alert=True)
    await admin_task_detail(callback)
...
async def admin_task_pause(callback: CallbackQuery):
    ...
    conn.commit()
    backup_db()
    await callback.answer("⏸ Задание приостановлено!", show_alert=True)
    await admin_task_detail(callback)
...
async def admin_task_resume(callback: CallbackQuery):
    ...
    conn.commit()
    backup_db()
    await callback.answer("▶️ Задание возобновлено!", show_alert=True)
    await admin_task_detail(callback)
...
async def admin_task_delete(callback: CallbackQuery):
    ...
    # Возвращаем монеты создателю за неиспользованные места
    cursor.execute("SELECT creator_id, max_executors, current_executors, reward_per_unit, is_elite FROM tasks WHERE id = ?", (task_id,))
    task = cursor.fetchone()
    if task:
        creator_id, max_exec, cur_exec, reward, is_elite = task
        unused = max_exec - cur_exec
        if unused > 0:
            refund_per_place = 250 if is_elite else 21
    ...
    await admin_tasks_show(callback)
...
# ---- ПРОВЕРКА ОТПИСОК (раз в 30 секунд) ----
async def check_unsubscribes():
    cycle_count = 0
    while True:
        try:
            # Проверяем выполнения заданий на отписки
            ...
            for exec_id, task_id, user_id, creator_id, link, reward, is_elite in executions:
                ...
                if not is_subscribed:
                    ...
                    try:
                        await bot.send_message(user_id, f"⚠️ ВНИМАНИЕ! Вы отписались от канала @{channel}!\n\nВы выполнили задание и получили награду {reward} монет.\nЕсли вы НЕ подпишетесь обратно в течение 10 минут:\n❌ Штраф: {reward * 2} монет\n❌ Заказчику вернутся его монеты\n\n👉 Подпишитесь обратно: https://t.me/{channel}")
                    except:
                        pass

                    try:
                        await bot.send_message(creator_id, f"⚠️ Пользователь @{user_id} отписался от вашего канала @{channel}!\n\n📋 Задание #{task_id}\n⏳ У него есть 10 минут, чтобы подписаться обратно.")
                    except:
                        pass

            # Проверяем отложенные штрафы (10 минут прошло?)
            ...
            for pen_id, user_id, task_id, exec_id, creator_id, reward, channel, start_time in pending:
                ...
                if (datetime.now() - start).total_seconds() >= 600:  # 10 минут
                    try:
                        member = await bot.get_chat_member(f"@{channel}", user_id)
                        is_subscribed = member.status not in ['left', 'kicked']
                    except:
                    ...
                    if not is_subscribed:
                        ...
                        try:
                            await bot.send_message(user_id, f"❌ ШТРАФ!\n\nВы не подписались обратно на канал @{channel} в течение 10 минут.\n💰 Штраф: -{penalty} монет\n📋 Задание #{task_id}")
                        except:
                            pass

                        try:
                            await bot.send_message(creator_id, f"✅ ВОЗВРАТ МОНЕТ!\n\nПользователь @{user_id} не подписался обратно на канал @{channel}.\n💰 Вам возвращено: +{reward} монет\n📋 Задание #{task_id}")
                        except:
                            pass
                    else:
                        try:
            ...
            # Закрываем просроченные задания
            ...
            # Авто-принятие Elite-заданий (через 2 дня игнора)
...
# ---- СБРОС НЕДЕЛЬНОЙ СТАТИСТИКИ (каждый понедельник в 00:00) ----
async def reset_weekly_stats():
    while True:
        ...
        if now.weekday() == 0 and now.hour == 0 and now.minute == 0:
            ...
            await asyncio.sleep(86400)  # Ждём сутки после сброса
        else:
            await asyncio.sleep(60)  # Проверяем раз в минуту

# ---- ТАЙМАУТЫ FSM (авто-сброс при бездействии > 10 минут) ----
async def check_fsm_timeouts():
    while True:
        now = datetime.now()
        to_clear = [uid for uid, t in list(fsm_last_activity.items()) if (now - t).total_seconds() > 600]
...
async def handle_any_message(message: Message, state: FSMContext):
    # Если есть активное FSM-состояние — не трогаем

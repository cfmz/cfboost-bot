...
def update_balance(user_id, amount, description, txn_type, commit=True):
    """Обновление баланса. Если commit=False, коммит должен быть сделан снаружи."""
...
async def menu_cmd(message: Message, state: FSMContext):
    await state.clear()
    fsm_last_activity.pop(message.from_user.id, None)
    await send_with_photo(message.from_user.id, "menu",
...
async def rating_cmd(message: Message, state: FSMContext):
    ...
    await send_with_photo(message.from_user.id, "rating",
...
# ========== ПЕРЕВОД МОНЕТ (с проверкой бана) ==========
...
async def take_elite_task(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Обновление баланса без коммита
        update_balance(user_id, 0, f"Взятие Elite-задания #{task_id}", "earn", commit=False)  # пока не начисляем, место просто занято
        cursor.execute(
...
# ========== ОТПРАВКА СКРИНШОТА ==========
# ... (весь код из предыдущей версии без изменений)
...
# ... (остальные шаги создания Elite-задания без изменений)
# ========== АДМИН-ПАНЕЛЬ ==========
...
# ========== РОЗЫГРЫШ (ручной и автоматический) ==========
...
async def run_giveaway_and_notify():
    """Общая функция проведения розыгрыша и уведомления победителей."""

...
def check_bot_in_channel(channel_username):
    ...
    try:
        ...
        # Проверяем, есть ли бот в админах
...
async def tasks_menu(message: Message):
    ...
    for i, task in enumerate(tasks[:10]):
        ...
        # Кнопка для взятия задания (только если есть свободные места и пользователь не выполнял)
...
async def take_task(callback: CallbackQuery):
    ...
    # Проверяем, если задание Elite — нужна подписка

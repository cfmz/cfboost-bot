...
async def check_required(callback: CallbackQuery):
    ...
    if not_sub:
        ...
        kb.append([{"text": "🔙 Главное меню", "callback_data": "main_menu", "style": "default"}])
        edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, f"❌ Вы не подписаны на: {', '.join(not_sub)}\n\nПодпишитесь и проверьте снова.", kb)
    else:
        user = get_user(user_id)
        if not user or user[13] == 0:
            ...
            conn.commit()
            backup_db()
            edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ Вы подписаны на все каналы!\n🎁 Вы получили 5000 баллов!", back_btn("main_menu"))
        else:
            edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ Вы уже получили бонус.", back_btn("main_menu"))
...
async def buy_elite(callback: CallbackQuery):
    ...
    conn.commit()
    backup_db()
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ Elite Sub активирована на 30 дней!", back_btn("main_menu"))
    await callback.answer()
...
async def rating_refs(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, back_btn("rating_back"))
    await callback.answer()
...
async def giveaway_top(callback: CallbackQuery):
    ...
    text += "\n\n🏆 Призы:\n🥇 10,000\n🥈 5,000\n🥉 3,000"
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, back_btn("giveaway_back"))
    await callback.answer()
...
async def check_earn(callback: CallbackQuery):
    ...
    if cursor.fetchone()[0] == 0:
        ...
        edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ +3500 баллов!", back_btn("main_menu"))
    else:
...
async def more_cmd(message: Message):
    ...
    if not channels:
        ...
        await send_with_photo(message, "more", "➕ Пока нет заданий.", kb)
        return
    kb = []
    for ch, name in channels:
        kb.append([InlineKeyboardButton(text=f"📢 {name}", url=f"https://t.me/{ch}")])
...
async def tasks_menu(message: Message):
    ...
    if not user:
        create_user(message.from_user.id, message.from_user.username, message.from_user.full_name)
        user = get_user(user_id)
    cursor.execute("SELECT id, creator_id, task_type, link, description, reward_per_unit, max_executors, current_executors, is_elite FROM tasks WHERE status = 'active' AND current_executors < max_executors")
...
async def take_task(callback: CallbackQuery):
    ...
    if is_elite and not is_elite_active(user_id):
        edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "🔒 Это Elite-задание. Требуется подписка Elite Sub.", [[{"text": "💎 Купить Elite Sub", "callback_data": "buy_elite", "style": "success"}]])
        await callback.answer()
        return
    ...
    if channel:
        try:
            ...
            if member.status in ['left', 'kicked']:
                edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, f"❌ Вы не подписаны на канал @{channel}. Подпишитесь и попробуйте снова.", back_btn("tasks_menu_back"))
                await callback.answer()
                return
        except Exception as e:
    ...
    if creator:
        try:
            ...
            pass
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, f"✅ ВЫПОЛНЕНО! +{reward} монет!\n\n📋 Задание #{task_id}\n📊 Осталось мест: {max_exec - (current_exec + 1)}", back_btn("tasks_menu_back"))
    try:
...
async def task_type_selected(callback: CallbackQuery, state: FSMContext):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📎 Введите ссылку (канал, пост или видео):", back_btn("tasks_menu_back"))
...
async def task_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    cursor.execute("INSERT INTO tasks (creator_id, task_type, link, description, reward_per_unit, max_executors, is_elite) VALUES (?, ?, ?, ?, ?, ?, ?)", (user_id, task_type, link, description, price_per, count, 0))
    conn.commit()
    backup_db()
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ Задание создано! Ожидайте исполнителей.", back_btn("main_menu"))
    await state.clear()
    await callback.answer()

@dp.callback_query(lambda c: c.data == "task_cancel")
async def task_cancel(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "❌ Создание задания отменено.", back_btn("tasks_menu_back"))
    await state.clear()
    await callback.answer()
...
async def admin_channels(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📢 Управление каналами:\n\nВыберите тип:", kb)
    await callback.answer()
...
async def admin_add_required(callback: CallbackQuery, state: FSMContext):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите username канала (без @):", back_btn("admin_required_channels"))
...
async def admin_remove_required(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "🗑 Выберите канал для удаления:", InlineKeyboardMarkup(inline_keyboard=kb))
    await callback.answer()
...
async def admin_add_earn(callback: CallbackQuery, state: FSMContext):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите username канала (без @):", back_btn("admin_earn_channels"))
    await state.set_state(AdminState.waiting_channel_username)
    await callback.answer()
...
async def admin_add_extra(callback: CallbackQuery, state: FSMContext):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите username канала (без @):", back_btn("admin_extra_channels"))
    await state.set_state(AdminState.waiting_channel_username)
    await callback.answer()
...
async def admin_promocodes(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "🎟 Управление промокодами:", kb)
    await callback.answer()

@dp.callback_query(lambda c: c.data == "admin_create_promo")
async def admin_create_promo(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите код промокода (латиница, цифры):", back_btn("admin_promocodes"))
...
async def admin_list_promo(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, back_btn("admin_promocodes"))
    await callback.answer()
...
async def admin_users(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "👤 Управление пользователями:", kb)
    await callback.answer()

@dp.callback_query(lambda c: c.data == "admin_give_elite")
async def admin_give_elite(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "💎 Введите ID пользователя для выдачи Elite Sub:", back_btn("admin_users"))
...
async def admin_take_elite(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "💎 Введите ID пользователя для забора Elite Sub:", back_btn("admin_users"))
    await state.set_state(AdminState.waiting_user_id)
...
async def admin_give_coins(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "💰 Введите ID пользователя:", back_btn("admin_users"))
    await state.set_state(AdminState.waiting_user_id)
...
async def admin_ban_user(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "🚫 Введите ID пользователя для блокировки:", back_btn("admin_users"))
    await state.set_state(AdminState.waiting_user_id)
...
async def admin_unban_user(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "🔓 Введите ID пользователя для разблокировки:", back_btn("admin_users"))
    await state.set_state(AdminState.waiting_user_id)
...
async def admin_user_id(message: Message, state: FSMContext):
    ...
    if action == "give_coins":
        ...
        send_colored_keyboard(message.chat.id, text, back_btn("admin_users"))
        await state.clear()
...
async def admin_broadcast(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📨 Рассылка:\n\nВыберите тип:", kb)
    await callback.answer()
...
async def admin_broadcast_all(callback: CallbackQuery, state: FSMContext):
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите текст рассылки:", back_btn("admin_broadcast"))
...
async def admin_stats(callback: CallbackQuery):
    ...
    edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, back_btn("admin_back"))
    await callback.answer()
...
async def check_unsubscribes():
    ...
    while True:
        try:
            cursor.execute("SELECT te.id, te.task_id, te.user_id, t.creator_id, t.link, t.reward_per_unit, t.is_elite, te.is_penalized FROM task_executions te JOIN tasks t ON te.task_id = t.id WHERE te.is_verified = 1 AND t.status = 'active'")
            ...
            for exec_id, task_id, user_id, creator_id, link, reward, is_elite, is_penalized in executions:
                ...
                if not is_subscribed:
                    if tracker_key not in unsubscribe_tracker:
                        unsubscribe_tracker[tracker_key] = {'exec_id': exec_id, 'task_id': task_id, 'user_id': user_id, 'creator_id': creator_id, 'reward': reward, 'channel': channel, 'time': datetime.now()}
                        try:
            ...
            for tracker_key in to_penalize:
                ...
                cursor.execute("INSERT INTO task_penalties (user_id, task_id, amount, reason) VALUES (?, ?, ?, ?)", (data['user_id'], data['task_id'], penalty, f"Отписка от {data['channel']}"))
                cursor.execute("UPDATE task_executions SET is_penalized = 1 WHERE id = ?", (data['exec_id'],))
                conn.commit()
                backup_db()
                try:
                    await bot.send_message(data['user_id'], f"❌ ШТРАФ!\n\nВы не подписались обратно на канал @{data['channel']} в течение 10 минут.\n💰 Штраф: -{penalty} монет\n📋 Задание #{data['task_id']}")
                except:
                    pass
                try:
                    await bot.send_message(data['creator_id'], f"✅ ВОЗВРАТ МОНЕТ!\n\nПользователь @{data['user_id'] or data['user_id']} не подписался обратно на канал @{data['channel']}.\n💰 Вам возвращено: +{data['reward']} монет\n📋 Задание #{data['task_id']}")
                except:
                    pass
                del unsubscribe_tracker[tracker_key]

...
def update_balance(user_id, amount, description, txn_type):
    ...
    if amount > 0:
        ...
        cursor.execute("UPDATE users SET spent_weekly = spent_weekly + ? WHERE user_id = ?", (abs(amount), user_id))
...
# ========== АСИНХРОННАЯ ОТПРАВКА ЦВЕТНЫХ КНОПОК ==========
def back_btn(cd="back_to_main"):
    return [{"text": "🔙 Назад", "callback_data": cd, "style": "default"}]

async def async_send_colored_keyboard(chat_id, text, buttons, parse_mode="HTML"):
    ...
    try:
        await async_post(f"{TELEGRAM_API_URL}/sendMessage", json_data=payload)
    except Exception as e:
        logger.error(f"Ошибка отправки цветных кнопок: {e}")

async def async_edit_colored_keyboard(chat_id, message_id, text, buttons, parse_mode="HTML"):
    keyboard = {"inline_keyboard": []}
    for row in buttons:
        keyboard_row = []
        for btn in row:
    ...
    try:
        await async_post(f"{TELEGRAM_API_URL}/editMessageText", json_data=payload)
    except Exception as e:
        logger.error(f"Ошибка редактирования цветных кнопок: {e}")

async def async_delete_message(chat_id, message_id):
    try:
...
async def start_cmd(message: Message):
        ...
        if message.text and "ref" in message.text:
            try:
                ...
                if referrer and referrer[0] != user_id:
                    ...
                    cursor.execute("UPDATE users SET referrals_count = referrals_count + 1 WHERE user_id = ?", (referrer[0],))
                    cursor.execute("UPDATE users SET referrals_weekly = referrals_weekly + 1 WHERE user_id = ?", (referrer[0],))
                    conn.commit()
                    backup_db()
            except:
                pass
    ...
    if channels:
        ...
        await send_with_photo(message, "start", welcome_text + "\n\n🎁 Получите бонус после обязательных подписок.", InlineKeyboardMarkup(inline_keyboard=kb))
    else:
        ...
        await send_with_photo(message, "start", welcome_text, None)
...
async def check_required(callback: CallbackQuery):
    ...
    if not_sub:
        ...
        kb.append([{"text": "🔙 Главное меню", "callback_data": "main_menu", "style": "default"}])
        await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, f"❌ Вы не подписаны на: {', '.join(not_sub)}\n\nПодпишитесь и проверьте снова.", kb)
    else:
        user = get_user(user_id)
        if not user or user[13] == 0:
            ...
            conn.commit()
            backup_db()
            await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ Вы подписаны на все каналы!\n🎁 Вы получили 5000 баллов!", back_btn("main_menu"))
        else:
            await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ Вы уже получили бонус.", back_btn("main_menu"))
...
async def profile_cmd(message: Message):
    ...
    await send_with_photo(message, "profile", text, kb)
...
async def refresh_profile(callback: CallbackQuery):
    ...
    await send_with_photo(callback, "profile", text, kb)
    await callback.answer()
# ---- ELITE SUB ----
...
async def buy_elite(callback: CallbackQuery):
    ...
    conn.commit()
    backup_db()
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ Elite Sub активирована на 30 дней!", back_btn("main_menu"))
    await callback.answer()
...
async def rating_refs(callback: CallbackQuery):
    ...
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, back_btn("rating_back"))
    await callback.answer()
...
async def transfer_cmd(message: Message, state: FSMContext):
    kb = [back_btn("back_to_extra")]
    await async_send_colored_keyboard(message.chat.id, "💸 Введите ID пользователя:", kb)
...
async def transfer_id(message: Message, state: FSMContext):
    main_buttons = ["💎 Elite Sub", "👤 Профиль", "📋 Задания", "⚡ Меню", "🪙 Заработать", "➕ Больше заданий", "📢 Поддержка", "📊 Статистика"]
    if message.text in main_buttons:
        await state.clear()
        if message.text == "💎 Elite Sub":
            await elite_cmd(message)
            ...
            await earn_cmd(message)
        elif message.text == "➕ Больше заданий":
...
async def giveaway_top(callback: CallbackQuery):
    ...
    text += "\n\n🏆 Призы:\n🥇 10,000\n🥈 5,000\n🥉 3,000"
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, back_btn("giveaway_back"))
    await callback.answer()
...
async def earn_cmd(message: Message):
    ...
    if not channels:
        ...
        await send_with_photo(message, "earn", "🪙 Пока нет заданий.", kb)
        return
    kb = []
    for ch, name in channels:
...
async def check_earn(callback: CallbackQuery):
    ...
    if cursor.fetchone()[0] == 0:
        ...
        await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ +3500 баллов!", back_btn("main_menu"))
    else:
...
async def more_cmd(message: Message):
    ...
    cursor.execute("SELECT id, creator_id, task_type, link, description, reward_per_unit, max_executors, current_executors, is_elite FROM tasks WHERE is_elite = 1 AND status = 'active' AND current_executors < max_executors")
    elite_tasks = cursor.fetchall()
    text = "👑 ELITE-РАЗДЕЛ\n━━━━━━━━━━━━━━━\n\n"
    ...
    if elite_tasks:
        text += "📋 ELITE-ЗАДАНИЯ:\n"
        for task in elite_tasks[:5]:
            task_id, creator_id, task_type, link, description, reward, max_exec, current_exec, is_elite = task
            free = max_exec - current_exec
            text += f"• #{task_id} {task_type}: {reward}₿ | Свободно: {free}/{max_exec}\n"
        text += "\n"
    if not sponsor_channels and not elite_tasks:
        text += "Пока нет активных заданий.\n"
    can_create = is_elite_active(user_id)
    kb = [
        [InlineKeyboardButton(text="📋 Список заданий", callback_data="more_tasks_list")],
    ]
    if can_create:
        kb.append([InlineKeyboardButton(text="➕ Создать Elite-задание", callback_data="create_elite_task")])
    else:
        kb.append([InlineKeyboardButton(text="🔒 Купить Elite Sub для создания", callback_data="buy_elite")])
...
async def tasks_menu(message: Message):
    ...
    for i, task in enumerate(tasks[:10]):
        ...
        if free > 0 and not already_done:
            kb.append([InlineKeyboardButton(text=f"✅ Взять задание #{i+1}", callback_data=f"take_task_{task_id}")])
...
async def take_task(callback: CallbackQuery):
    ...
    if is_elite and not is_elite_active(user_id):
        await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "🔒 Это Elite-задание. Требуется подписка Elite Sub.", [[{"text": "💎 Купить Elite Sub", "callback_data": "buy_elite", "style": "success"}]])
        await callback.answer()
        return
    ...
    if channel:
        try:
            ...
            if member.status in ['left', 'kicked']:
                await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, f"❌ Вы не подписаны на канал @{channel}. Подпишитесь и попробуйте снова.", back_btn("tasks_menu_back"))
                await callback.answer()
                return
        except Exception as e:
    ...
    if creator:
        try:
            ...
            pass
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, f"✅ ВЫПОЛНЕНО! +{reward} монет!\n\n📋 Задание #{task_id}\n📊 Осталось мест: {max_exec - (current_exec + 1)}", back_btn("tasks_menu_back"))
    try:
...
# ---- СПИСОК ELITE-ЗАДАНИЙ ----
@dp.callback_query(lambda c: c.data == "more_tasks_list")
async def more_tasks_list(callback: CallbackQuery):
    user_id = callback.from_user.id
    cursor.execute("SELECT id, creator_id, task_type, link, description, reward_per_unit, max_executors, current_executors, is_elite FROM tasks WHERE status = 'active' AND current_executors < max_executors AND is_elite = 1")
    tasks = cursor.fetchall()
    if not tasks:
        await callback.answer("❌ Нет активных Elite-заданий.", show_alert=True)
        return
    text = "👑 ELITE-ЗАДАНИЯ:\n\n"
    kb = []
    for i, task in enumerate(tasks[:10]):
        ...
        if not already_taken and free > 0:
            kb.append([InlineKeyboardButton(text=f"✅ Взять задание #{i+1}", callback_data=f"take_elite_task_{task_id}")])
...
async def take_elite_task(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📸 Отправить скриншот", callback_data=f"elite_submit_{task_id}")],
        [InlineKeyboardButton(text="🔙 К списку", callback_data="more_tasks_list")]
    ])
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, f"👑 Задание #{task_id} взято!\n\n📌 Требуется отправить скриншот выполнения.\n💰 Награда: {reward} монет\n📝 Описание: {description}", kb)
    await callback.answer()
...
async def elite_submit(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="more_tasks_list")]
    ])
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📸 Отправьте скриншот выполнения задания (фото или документ):", kb)
...
async def elite_rework(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id) and callback.from_user.id != int(callback.data.split("_")[2]):
        await callback.answer("⛔ Только заказчик может отправить на доработку.", show_alert=True)
        return
    _, _, task_id, user_id = callback.data.split("_")
    task_id = int(task_id)
    user_id = int(user_id)
    ...
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите сообщение для исполнителя (что нужно исправить):", back_btn("more_tasks_list"))
    await state.set_state(EliteSubmitState.waiting_rework_message)
    await callback.answer()

@dp.message(EliteSubmitState.waiting_rework_message, F.text)
async def elite_rework_message(message: Message, state: FSMContext):
    if message.text == "🔙 Отмена":
        await state.clear()
        await more_tasks_list(message)
        return
    data = await state.get_data()
    task_id = data['task_id']
    ...
    if rework_count == 3:
        ...
        await bot.send_message(message.from_user.id, f"⚠️ ВНИМАНИЕ! Вы отправили на доработку 3-й раз!\n\nЭто последняя доработка перед автоматическим зачислением баллов исполнителю.\nЕсли вы отправите на доработку ещё раз — задание будет АВТОМАТИЧЕСКИ ЗАКРЫТО.\n\nЗлоупотребление доработками → блокировка аккаунта.")
...
async def elite_message(callback: CallbackQuery, state: FSMContext):
    ...
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📝 Введите сообщение для исполнителя:", back_btn("more_tasks_list"))
    await state.set_state(EliteSubmitState.waiting_custom_message)
    await callback.answer()

@dp.message(EliteSubmitState.waiting_custom_message, F.text)
...
async def more_my_tasks(callback: CallbackQuery):
    ...
    kb = [[InlineKeyboardButton(text="🔙 Назад", callback_data="more_back")]]
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, text, kb)
    await callback.answer()
...
async def create_elite_task_start(callback: CallbackQuery, state: FSMContext):
    ...
    await async_delete_message(callback.message.chat.id, callback.message.message_id)
    await send_with_photo(callback.message, "create_task", "👑 Создание Elite-задания\n\nВыберите тип:", kb)
...
async def elite_type_selected(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(task_type=task_type)
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "📎 Введите ссылку (канал, пост или видео):", back_btn("more_back"))
...
async def elite_task_count(message: Message, state: FSMContext):
    ...
    if user[3] < total_cost:
        ...
        await state.clear()
        return
    text = f"👑 ПОДТВЕРЖДЕНИЕ ELITE-ЗАДАНИЯ\n━━━━━━━━━━━━━━━\n\nТип: {task_type.capitalize()}\nСсылка: {link}\nОписание: {description}\nКоличество: {count}\nНаграда: {reward}₿\nЦена заказчика: 250₿ × {count} = {total_cost}₿\nКомиссия: 10% от награды\n\n⚠️ ПРАВИЛА:\n• Вы можете отправить на доработку максимум 3 раза\n• После 3-й доработки баллы зачисляются АВТОМАТИЧЕСКИ\n• Злоупотребление доработками — блокировка"
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def elite_task_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    user = get_user(user_id)
    if user[3] < total_cost:
        await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "❌ Недостаточно монет!", back_btn("main_menu"))
        await state.clear()
        await callback.answer()
        return
    ...
    conn.commit()
    backup_db()
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "✅ Elite-задание создано! Ожидайте исполнителей.", back_btn("main_menu"))
    await state.clear()
    await callback.answer()

@dp.callback_query(lambda c: c.data == "elite_task_cancel")
async def elite_task_cancel(callback: CallbackQuery, state: FSMContext):
    await async_edit_colored_keyboard(callback.message.chat.id, callback.message.message_id, "❌ Создание отменено.", back_btn("more_back"))
    await state.clear()
    await callback.answer()
...
# ========== ПРОВЕРКА ОТПИСОК (ОПТИМИЗИРОВАНА) ==========
